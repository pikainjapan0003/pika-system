...
## BATCH-18 流程補充（2026-07-31）

| 入口／路由                      | 新增或確認的操作                         | 步數 | 狀態與界線                                                              |
| ------------------------------- | ---------------------------------------- | ---: | ----------------------------------------------------------------------- |
| `/customers/:customerId`        | 查看購物金餘額與流水；預覽後發放／調整   |    2 | owner-only；負調整不得使餘額小於 0；確認後才寫入                        |
| `/orders`                       | 綁定客戶後輸入購物金折抵並建立後台訂單   |    1 | 使用既有建單交易；折抵定格於訂單，取消時自動回沖                        |
| `/orders` 的賣貨便匯出區        | 日期篩選→檢查資格→勾選→明文二次確認→下載 |    4 | owner-only；只有符合資格的未出貨訂單可匯出；可下載 CSV 或官方 XLSM 格式 |
| `/orders` 的撿貨／出貨工具      | 開撿貨單後逐品項勾選／取消勾選           |  1–N | 勾選會持久保存；不改訂單狀態、不代表出貨                                |
| `/customers/:customerId` 的詳情 | 查看該客訂單歷史與定格毛利               |    0 | pending／免攤／尚無快照仍分流顯示；購物金不會改寫既有成本或毛利快照     |

上述操作沒有新增公開頁路由；客人端回應仍不包含購物金餘額、流水、成本或毛利。
