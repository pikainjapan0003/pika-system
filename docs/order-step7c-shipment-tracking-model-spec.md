...
### 方案 B：獨立 shipment_trackings 表（Snapshot + 控制欄位）

新增 `shipment_trackings` 表，但不記錄事件 timeline：

```
shipment_trackings 表（新增）：
  id, orderId (FK → orders.id), trackingCode, trackingProvider
  latestStatus, latestStatusAt
  lastCheckedAt, failureCount, checkError, isActive
  createdAt, updatedAt
```

| 優點 | 缺點 |
|------|------|
| orders 表保持簡潔 | 仍無法支援 Step 7E timeline |
| 可追蹤每次查詢控制狀態 | 未來加 events 表仍需額外 migration |
| orderId 可有多筆（換 trackingCode 後保留歷史記錄）| 比方案 A 複雜但功能不完整 |

**結論：作為過渡方案可接受，但不如直接上方案 C。**

---

### 方案 C：shipment_trackings + shipment_tracking_events（完整 history）
...
**結論：推薦採用。** 雖然初期資料為空，但資料模型正確性最高，避免日後破壞性重構。
...
| `trackingStatus` | text NOT NULL | 追蹤任務狀態（見下表）| `pending` |
...
|-------------|------|---------------------|
| `shipment_created` | 物流建單成功 | `preparing` |
| `in_transit` | 運送中 | `shipped` |
| `arrived_at_store` | 已到達取件門市 | `arrived` |
| `picked_up` | 客人已取件 | `picked_up` |
| `return_initiated` | 退件中 | `returned` |
| `returned` | 已退回寄件方 | `returned` |
| `exception` | 異常（地址錯誤、遺失等）| 維持原狀 |
| `unknown` | 無法對應的業者狀態 | 維持原狀 |
...
- 成功查詢後：根據 `latestEventStatus` 動態調整
  ...
  - 到達門市（`arrived_at_store`）：`nextCheckAt = NOW() + 30 minutes`（客人即將取件）
  - 已取件（`picked_up`）：停止查詢
...
### 8.3 Worker 內部 API（Step 7D 施工）

Worker 直接操作 DB，不需要 HTTP API（Drizzle ORM 直接呼叫）。若需要獨立 microservice 架構，再評估。
...
Step 7E 擴充公開 API 時，`trackingEvents` **不得包含**：

- 收件人姓名（即使業者 API 回傳了）
- 完整收件地址（eventLocation 只允許門市名稱或城市）
- 電話號碼
- 任何可辨識收件人身份的資訊
...
*文件版本：Step 7C Spec v1.0*
*撰寫日期：2026-06-07*
*撰寫：Claude B（Fixed Latest File Mode）*
*現況依據：docs/order-step7b-tracking-import-spec.md（main HEAD 4258c54）、lib/db/src/schema/orders.ts*
*分支：qa/step6f-cvs-store-selection-browser-mobile（main 基準：4258c54）*
