...
| Manual Preview All Providers QA | provider policy consistency | ✅ **PASS** | postoffice / tcat / 7-11 均確認為 Level 1；familymart 確認為 Level 3 |
| Manual Preview All Providers QA | 7-11 Published UI QA | ✅ **PASS** | 沿用 Step 7O 使用者截圖驗收結果 |
| Manual Preview All Providers QA | postoffice Published UI QA | ⚠️ **NOT RUN** | Step 7P 未執行；控制環境 E2E 驗收過；如需驗收可另開任務 |
| Manual Preview All Providers QA | tcat Published UI QA | ⚠️ **NOT RUN** | Step 7P 未執行；控制環境 E2E 驗收過；如需驗收可另開任務 |
...
- postoffice：Level 1 manual preview-only confirmed（production E2E 完成；Published UI 未 Step 7P 驗收）
- tcat：Level 1 manual preview-only confirmed（production E2E 完成；#36 one-shot done；Published UI 未 Step 7P 驗收）
...
postoffice / tcat Published UI 未在 Step 7P 驗收；如需驗收須另開任務。
```
...
### 可選：Step 7P-POSTOFFICE-TCAT-PUBLISHED-UI-QA

目的：如使用者需要補驗 postoffice / tcat Published UI，可另開此任務；不影響 Step 7P COMPLETED 狀態。
