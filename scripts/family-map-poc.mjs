...
const {
  firefox,
} = require("/home/runner/.npm/_npx/e41f203b7505f1fb/node_modules/playwright-core");
...
async function fetchDistrict(page, city, district) {
  ...
  page.on("request", (req) => {
    const url = req.url();
    if (
      url.includes("familyShop") ||
      url.includes("ShopList") ||
      url.includes("family.com.tw/net/")
    ) {
      ...
      await page.evaluate(
        (cid, cityName) => {
          const el =
            ...
            document.querySelector(`[name="${cid}"]`);
          if (el) {
            // Find the matching option
            const opts = Array.from(el.options);
            const match = opts.find(
              (o) => o.text.includes(cityName) || o.value.includes(cityName),
            );
            if (match) {
    ...
    for (const townParam of districtParamOptions[district] || [district]) {
      ...
      try {
        const res = await fetch(
          `https://api.map.com.tw/net/familyShop.aspx?type=ShopList&city=${encodeURIComponent(cityParam)}&town=${encodeURIComponent(townParam)}&fun=testCB`,
          {
            headers: {
  ...
  try {
    ...
    const pageInfo = await page.evaluate(() => ({
      ...
      selects: Array.from(document.querySelectorAll("select")).map((s) => ({
        ...
        firstFewOpts: Array.from(s.options)
          .slice(0, 3)
...
async function main() {
  ...
  console.log(
    "[family-map-poc] Phase 1: Direct API attempts with Referer spoofing...",
  );
  const directResults = [];
  ...
  console.log(
    "[family-map-poc] Phase 2: Playwright headless browser investigation...",
  );
  ...
  try {
    ...
    try {
      ...
      const structure = await page.evaluate(() => {
        ...
        const scripts = Array.from(document.querySelectorAll("script[src]"))
          ...
          .filter(
            (s) =>
              s.includes("map") || s.includes("shop") || s.includes("fami"),
          );
        const links = Array.from(document.querySelectorAll("a[href]"))
          .map((a) => a.href)
          .filter((h) => h.includes("map") || h.includes("shop"))
          .slice(0, 10);
      ...
      mainPageResult = {
        structure,
        capturedReqs: capturedReqs.slice(0, 30),
        capturedResps,
      };
    } catch (e) {
      mainPageResult = {
    ...
    iframePage.on("request", (req) => {
      ...
      if (url.includes("familyShop") || url.includes("ShopList")) {
        iframeCaptures.push(
          url.replace(/key=[^&]+/gi, "key=***REDACTED***").substring(0, 400),
        );
      }
    });
    ...
    try {
      ...
      if (pageState.iframes.length > 0) {
        const mapIframeSrc = pageState.iframes.find(
          (f) =>
            f.src &&
            (f.src.includes("map") ||
              f.src.includes("Map") ||
        ...
        if (mapIframeSrc) {
          ...
          await iframePage.goto(mapIframeSrc.src, {
            waitUntil: "domcontentloaded",
            timeout: 25000,
          });
          await waitMs(5000);
      ...
      const searchTriggered = await iframePage.evaluate(async () => {
        ...
        if (cityEl) {
          ...
          const taipeiOpt = opts.find(
            (o) => o.text.includes("台北市") || o.value.includes("台北"),
          );
          if (taipeiOpt) {
            ...
            return {
              found: true,
              selectedValue: taipeiOpt.value,
              selectedText: taipeiOpt.text,
            };
          }
        }
        return {
          found: false,
          allSelects: Array.from(document.querySelectorAll("select")).map(
            (s) => s.id + "/" + s.name,
          ),
        };
      });
        ...
        for (const resp of iframeResponses) {
          try {
            ...
            if (match) {
              ...
              resp.parsedCount = Array.isArray(parsed)
                ? parsed.length
                : parsed.Shop
                  ? parsed.Shop.length
                  : "unknown";
    ...
    const report = {
      ...
      officialEndpointObservations: {
        ...
        storeApiParams:
          "type=ShopList&city=<city>&town=<district>&fun=<callback>&key=***REDACTED***",
        dataFields: [
          "pkey (店舖號)",
          "NAME (店名)",
          "SERID (服務編號)",
        ...
        coordinateSystem:
          "px/py format exists but coordinate system unconfirmed (likely TWD97/TM2)",
      },
      currentDbSummary: {
        familyTotal: 2509,
        sevenTotal: 7386,
      ...
      gapAnalysis: {
        ...
        gapPct: "44%",
        hypothesis:
          "twcoupon.com source does not have complete national coverage",
      },
    };
    ...
    if (report.canAccessData) {
      report.recommendedNextStep =
        "F5-2: Use Playwright to do a dry-run scrape of all Taiwan districts from family.com.tw map — do NOT write to DB yet";
    } else {
      report.recommendedNextStep =
        'Option A: Install @playwright/chromium + build a full headless scraper that navigates the FamilyMart map and selects each city/district. OR Option B: Find a FamilyMart API key via browser DevTools on a real device and replicate. OR Option C: Keep 2,509 stores and show "data may be incomplete" warning in UI.';
    }

    const outputPath = resolve(
      "/home/runner/workspace/data/cvs/family-official-map-poc-stepf5.json",
    );
    ...
    console.log(
      `[family-map-poc] iframeResponses count: ${iframeResponses.length}`,
    );
    console.log(
      `[family-map-poc] iframeCaptures count: ${iframeCaptures.length}`,
    );
