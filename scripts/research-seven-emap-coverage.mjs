...
function buildPayloads(region) {
  return [
    ...
    {
      label: "StoreName=縣市+區名",
      payload: { StoreName: region.keywords[1] || region.keywords[0] },
    },
    // 地址類
    {
      label: "address=縣市+行政區",
      payload: { address: `${region.city}${region.district.slice(0, 2)}` },
    },
    { label: "address=縣市全稱", payload: { address: region.city } },
    // city/town 分離
    { label: "city=縣市", payload: { city: region.city } },
    {
      label: "city+town=縣市+區",
      payload: { city: region.city, town: region.district },
    },
    {
      label: "city+town=縣市+區短",
      payload: { city: region.city, town: region.district.slice(0, -1) },
    },
    // roadname
    { label: "roadname=行政區", payload: { roadname: region.district } },
    // leftMenuChecked
    {
      label: "leftMenuChecked=1",
      payload: { StoreName: region.keywords[0], leftMenuChecked: "1" },
    },
    // SpecialStore_Kind
    {
      label: "SpecialStore_Kind=空",
      payload: { StoreName: region.keywords[0], SpecialStore_Kind: "" },
    },
...
for (const region of REGIONS) {
  ...
  for (const { label, payload } of payloads) {
    ...
    const inDb = res.results.filter(
      (r) => r.poiId && dbIds.has(r.poiId),
    ).length;
    const newOnes = res.results.filter((r) => r.poiId && !dbIds.has(r.poiId));
    const newCount = newOnes.length;

    // 跨區偵測：結果中不屬於本區的
    const crossRegion = res.results.filter(
      (r) => r.city && r.city !== region.city,
    ).length;
    ...
    console.log(
      `  [${label}] → ${n} 筆（DB已有=${inDb}, 新=${newCount}, 跨區=${crossRegion}）${limitStr}${errStr}`,
    );
    if (n > 0 && n <= 5) {
      res.results.forEach((r) =>
        console.log(`    • ${r.poiId} ${r.name} ${r.address}`),
      );
    }
  }
...
strategyRows.forEach((r) => {
  const flags = [
    ...
    .join(" ");
  console.log(
    `  ${r.label.padEnd(22)} avg結果=${r.avgResults.padStart(5)}  avg新=${r.avgNew.padStart(5)}  ${flags}`,
  );
});
