...
if (res.ok) {
  ...
  check(
    ...
    typeof res.normalizedStatus === "string" && res.normalizedStatus.length > 0,
  );
  check(
    "events ordered oldest→latest",
    !res.events[0].occurredAt ||
      !res.events.at(-1).occurredAt ||
      res.events[0].occurredAt <= res.events.at(-1).occurredAt,
  );
  console.log(
    `  → normalizedStatus=${res.normalizedStatus} latest="${res.latestStatusText}" at=${res.latestEventAt} events=${res.events.length}`,
  );
  const dump = JSON.stringify(res);
  check(
    "no customer PII keys in output",
    !/RCV_USER_NAME|buyerName|buyerPhone/i.test(dump),
  );
}
...
for (const bad of ["", "abc-123"]) {
  ...
  check(
    `invalid code "${bad}": ok=false INVALID_TRACKING_CODE`,
    !r.ok && r.errorCode === "INVALID_TRACKING_CODE",
    JSON.stringify(r),
  );
}
...
check(
  "fake code: NO_RESULT or REMOTE_ERROR",
  !fake.ok && ["NO_RESULT", "REMOTE_ERROR"].includes(fake.errorCode),
  JSON.stringify(fake),
);
...
check(
  "timeoutMs=1: ok=false TIMEOUT, retryable",
  !t.ok && t.errorCode === "TIMEOUT" && t.retryable === true,
  JSON.stringify(t),
);
