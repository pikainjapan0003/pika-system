...
import { runControlledWorkerBatch } from "../../artifacts/api-server/src/lib/logistics/workers/multiProviderDryRunWorker.ts";
...
check(
  "wouldWriteEvents=5",
  po.jobs[0]?.wouldWriteEvents === 5,
  `got ${po.jobs[0]?.wouldWriteEvents}`,
);
check("latestStatusText=投遞成功", po.jobs[0]?.latestStatusText === "投遞成功");
check(
  "input passthrough (trackingId/orderId/storeId)",
  po.jobs[0]?.trackingId === 1 &&
    po.jobs[0]?.orderId === 101 &&
    po.jobs[0]?.storeId === 9,
);
...
check(
  "wouldWriteEvents=5",
  tc.jobs[0]?.wouldWriteEvents === 5,
  `got ${tc.jobs[0]?.wouldWriteEvents}`,
);
...
check(
  "tcat keys no collision (location in key)",
  new Set(tcKeys).size === tcKeys.length,
);
check(
...
check(
  "successCount=2",
  mixed.successCount === 2,
  JSON.stringify(mixed.providerSummary),
);
check(
  "dryRun=true / noWrite=true",
  mixed.dryRun === true && mixed.noWrite === true,
);
check("rateLimitApplied=true", mixed.rateLimitApplied === true);
check("appliedDelayMs=300", mixed.appliedDelayMs === 300);
check(
  "sleep called once between 2 jobs",
  sleepCalls.length === 1 && sleepCalls[0] === 300,
...
check(
  "711 skippedReason=CONTROLLED_WORKER_DISABLED",
  se.jobs[0]?.skippedReason?.startsWith("CONTROLLED_WORKER_DISABLED") === true,
);
check(
  "711 no external call (adapterOk=false)",
  se.jobs[0]?.adapterOk === false && se.jobs[0]?.wouldWriteEvents === 0,
);
...
check(
  ...
  guard.jobs[0]?.skippedReason?.startsWith("UNSUPPORTED_PROVIDER") === true,
);
check(
  "empty trackingCode skipped",
  guard.jobs[1]?.skippedReason === "EMPTY_TRACKING_CODE",
);
...
check(
  "adapter called only 2 times (then circuit open)",
  cbCalls === 2,
  `got ${cbCalls}`,
);
check(
  "jobs 3,4 circuitBreakerSkipped",
  cb.jobs[2]?.circuitBreakerSkipped === true &&
    cb.jobs[3]?.circuitBreakerSkipped === true,
);
check(
  "providerSummary.tcat.circuitBreakerTriggered=true",
  cb.providerSummary.tcat?.circuitBreakerTriggered === true,
);
check(
  "failedCount=2, skippedCount=2",
  cb.failedCount === 2 && cb.skippedCount === 2,
);
...
check(
  "all 3 retryable jobs executed (no breaker)",
  rtCalls === 3,
  `got ${rtCalls}`,
);
check(
  "circuitBreakerTriggered=false",
  rt.providerSummary.postoffice?.circuitBreakerTriggered === false,
);
check(
  "failedCount=3, skippedCount=0",
  rt.failedCount === 3 && rt.skippedCount === 0,
);
...
check(
  "sleep called 3 times (between external calls only, skip 不計)",
  sleepCalls.length === 3,
  JSON.stringify(sleepCalls),
);
...
check(
  "no db.insert/db.update/db.select",
  !/\bdb\.(insert|update|select|delete)\b/.test(workerSrc),
);
