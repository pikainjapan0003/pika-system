...
const { eq, and } = await import(pathToFileURL(path.join(ROOT, "artifacts/api-server/node_modules/drizzle-orm/index.js")));
...
async function makeTestOrder(trackingCode) {
  ...
  const [order] = await db.insert(ordersTable).values({
    ...
    buyerName: "SMOKE-TEST", buyerPhone: "0900000000", pickupMethod: "convenience_store",
    unitPrice: "100", totalPrice: "100",
  }).returning({ id: ordersTable.id });
  createdOrderIds.push(order.id);
  const [tracking] = await db.insert(shipmentTrackingsTable).values({
    orderId: order.id, trackingCode, trackingProvider: "familymart",
...
let knownId, knownIsTestData = false;
const [existing] = await db.select({ id: shipmentTrackingsTable.id }).from(shipmentTrackingsTable)
  .where(and(eq(shipmentTrackingsTable.trackingProvider, "familymart"), eq(shipmentTrackingsTable.trackingCode, KNOWN)));
if (existing) {
  knownId = existing.id;
  console.log(`known code ${mask(KNOWN)} 已存在 tracking #${knownId}（沿用既有資料，更新即正式貨態）`);
} else {
...
try {
  ...
  const dry = await runFamilyMartTrackingWorker({ trackingIds: [knownId], dryRun: true });
  check("dryRun: totalJobs = 1", dry.totalJobs === 1, JSON.stringify(dry));
  check("dryRun: success", dry.results[0]?.status === "success", JSON.stringify(dry.results));
  check("dryRun: normalizedStatus 有值", Boolean(dry.results[0]?.normalizedStatus));
  check("dryRun: runLogId = null", dry.runLogId === null);
  const afterDry = await getTracking(knownId);
  check("dryRun: 不寫 shipment_trackings",
    String(afterDry.lastCheckedAt) === String(before.lastCheckedAt) && afterDry.trackingStatus === before.trackingStatus);
  const eventsAfterDry = await countEvents(knownId);
  console.log(`  → dryRun normalizedStatus=${dry.results[0]?.normalizedStatus} latest="${dry.results[0]?.latestStatusText}"`);

  // --- 2. 實跑：更新 trackings + events + run log ---
  const run1 = await runFamilyMartTrackingWorker({ trackingIds: [knownId] });
  check("run1: success", run1.results[0]?.status === "success", JSON.stringify(run1.results));
  check("run1: runLogId 有值", typeof run1.runLogId === "number");
  const t1 = await getTracking(knownId);
  check("run1: tracking_status 更新（非 pending）", t1.trackingStatus !== "pending" && ["active", "delivered"].includes(t1.trackingStatus), t1.trackingStatus);
  check("run1: latest_event_description 有值", Boolean(t1.latestEventDescription), t1.latestEventDescription);
  ...
  check("run1: events 寫入", ev1 > eventsAfterDry, `before=${eventsAfterDry} after=${ev1}`);
  const [log1] = await db.select().from(shipmentTrackingRunLogsTable).where(eq(shipmentTrackingRunLogsTable.id, run1.runLogId));
  check("run1: run log status = success", log1?.status === "success", log1?.status);
  check("run1: run log counts", log1?.totalJobs === 1 && log1?.successCount === 1 && log1?.failedCount === 0);
  ...
  check("run2: insertedEventCount = 0", run2.results[0]?.insertedEventCount === 0, run2.results[0]?.insertedEventCount);
  const t2 = await getTracking(knownId);
  check("run2: tracking 狀態保持正確", t2.trackingStatus === t1.trackingStatus && t2.failureCount === 0);
  ...
  check("fake: failed", runF.results[0]?.status === "failed", JSON.stringify(runF.results));
  check("fake: errorCode = NO_RESULT", runF.results[0]?.errorCode === "NO_RESULT", runF.results[0]?.errorCode);
  ...
  check("fake: tracking_status = failed（non-retryable）", tf.trackingStatus === "failed", tf.trackingStatus);
  const exs = await db.select().from(shipmentTrackingExceptionsTable)
    .where(eq(shipmentTrackingExceptionsTable.shipmentTrackingId, fakeId));
  check("fake: exception 建立", exs.length === 1, exs.length);
  check("fake: exception open/error/non-retryable", exs[0]?.status === "open" && exs[0]?.severity === "error" && exs[0]?.retryable === false);
  const [logF] = await db.select().from(shipmentTrackingRunLogsTable).where(eq(shipmentTrackingRunLogsTable.id, runF.runLogId));
  check("fake: run log status = failed", logF?.status === "failed", logF?.status);
  check("fake: run log errorSummary 含 NO_RESULT", /NO_RESULT/.test(logF?.errorSummary ?? ""), logF?.errorSummary);
} finally {
  ...
  if (createdOrderIds.length) console.log(`已清除 ${createdOrderIds.length} 筆測試 order（cascade tracking/events）`);
  await pool.end();
}
