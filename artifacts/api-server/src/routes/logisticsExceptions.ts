 ...
 */
router.get("/stores/:storeId/logistics/exceptions", requireAuth, async (req: any, res: any) => {
  ...
  if (status !== "all" && !STATUSES.has(status))
    return fail(res, 400, "INVALID_STATUS", "status must be open|reviewing|resolved|ignored|all");
  if (provider !== "all" && !PROVIDERS.has(provider))
    return fail(res, 400, "INVALID_PROVIDER", "provider must be 711|familymart|tcat|postoffice|all");
  if (sourceType !== "all" && !SOURCE_TYPE_ALIASES[sourceType])
    return fail(res, 400, "INVALID_SOURCE_TYPE", "sourceType must be import|worker|manual|agent|all");

  const conditions = [eq(shipmentTrackingExceptionsTable.storeId, storeId)];
  if (status !== "all") conditions.push(eq(shipmentTrackingExceptionsTable.status, status));
  if (provider !== "all") conditions.push(eq(shipmentTrackingExceptionsTable.provider, provider));
  if (sourceType !== "all")
    conditions.push(eq(shipmentTrackingExceptionsTable.sourceType, SOURCE_TYPE_ALIASES[sourceType]));
  ...
  try {
    const items = await db
      .select({
        ...
        shipmentTrackingId: shipmentTrackingExceptionsTable.shipmentTrackingId,
    ...
    return fail(res, 500, "EXCEPTIONS_READ_FAILED", "Could not load exceptions");
  }
});

/** 更新異常狀態：resolved / ignored 設 resolved_at + resolved_by；open 清空。 */
router.patch("/stores/:storeId/logistics/exceptions/:id", requireAuth, async (req: any, res: any) => {
  const storeId = parseInt(req.params.storeId);
  ...
  if (status !== "resolved" && status !== "ignored" && status !== "open")
    return fail(res, 400, "INVALID_STATUS", "status must be resolved|ignored|open");
  ...
  try {
    const [updated] = await db
      ...
      .set(
        status === "open"
          ...
          : { status, resolvedAt: new Date(), resolvedBy: req.userId ?? "owner-ui" },
      )
      .where(
        and(
    ...
    return fail(res, 500, "EXCEPTIONS_UPDATE_FAILED", "Could not update exception");
  }
});
  ...
  async (req: any, res: any) => {
    ...
    try {
      ...
      if (exception.provider !== "familymart")
        return fail(res, 400, "PROVIDER_NOT_SUPPORTED", "目前僅支援全家重新查詢");
      ...
      if (trackingId == null)
        return fail(res, 400, "TRACKING_NOT_FOUND", "找不到可重新查詢的物流追蹤資料");
      ...
      return fail(res, 500, "EXCEPTION_RETRY_FAILED", "Could not retry exception");
    }
  },
);
