...
before(async () => {
  ...
  orderMatchId = (await mkOrder("林模賢", "0970000090", "全家大里大發店")).rows[0].id;
  orderOtherId = (await mkOrder("測模試", "0911000022", "全家不存在店")).rows[0].id;
});

after(async () => {
  await pool.query(`DELETE FROM shipment_tracking_run_logs WHERE store_id = $1`, [storeId]);
  await pool.query(`DELETE FROM stores WHERE id = $1`, [storeId]); // cascades orders/batches/rows/exceptions/trackings
  server?.close();
  await pool.end();
});

function upload({ provider, filePath, fileName, contentType, auth = true, omitFile = false }) {
  const form = new FormData();
  if (provider != null) form.append("provider", provider);
  if (!omitFile) {
    form.append(
      "file",
      new Blob([readFileSync(filePath)], { type: contentType ?? "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }),
      fileName
    );
  }
...
function confirm(batchId, body, userId = TEST_USER) {
  return fetch(`${baseUrl}/stores/${storeId}/logistics/imports/${batchId}/confirm`, {
    method: "POST",
...
const famiUpload = () => upload({ provider: "familymart", filePath: path.join(FIXTURES, famiFile), fileName: famiFile });

describe("logistics import dry-run persistence + confirm", () => {
  test("requires auth", async () => {
    const res = await upload({ provider: "711", filePath: path.join(FIXTURES, sevenFile), fileName: sevenFile, auth: false });
    assert.equal(res.status, 401);
  });

  test("7-11 dry-run still parses (regression)", async () => {
    const res = await upload({ provider: "711", filePath: path.join(FIXTURES, sevenFile), fileName: sevenFile });
  ...
  test("invalid provider / missing file / wrong extension / wrong columns", async () => {
    ...
    assert.equal((await (await upload({ provider: "tcat", filePath: seven, fileName: sevenFile })).json()).errorCode, "INVALID_PROVIDER");
    assert.equal((await (await upload({ provider: "711", omitFile: true })).json()).errorCode, "MISSING_FILE");
    assert.equal((await (await upload({ provider: "711", filePath: seven, fileName: "a.csv", contentType: "text/csv" })).json()).errorCode, "UNSUPPORTED_FILE_TYPE");
    assert.equal((await (await upload({ provider: "familymart", filePath: seven, fileName: sevenFile })).json()).errorCode, "REQUIRED_COLUMNS_MISSING");
  });
  ...
  test("dry-run persists batch + rows with masked PII only", async () => {
    ...
    assert.ok(body.dryRun.matchedRows >= 1, "expected the synthetic order to match");

    const batch = await pool.query(`SELECT * FROM logistics_import_batches WHERE id = $1`, [body.batchId]);
    ...
    const rows = await pool.query(`SELECT * FROM logistics_import_rows WHERE batch_id = $1`, [body.batchId]);
    assert.equal(rows.rows.length, 172);
    for (const r of rows.rows) {
      if (r.recipient_name_masked) assert.ok(r.recipient_name_masked.includes("*"), "name must be masked");
      if (r.recipient_phone_masked) assert.ok(r.recipient_phone_masked.includes("*"), "phone must be masked");
      const raw = JSON.stringify(r.raw_row_json ?? {});
      assert.ok(!raw.includes("林模賢") && !raw.includes("0970000090"), "raw_row_json must not contain raw PII");
    }
  });
  ...
  test("confirm imports matched rows and updates everything", async () => {
    ...
    const order = await pool.query(`SELECT tracking_code, tracking_provider FROM orders WHERE id = $1`, [orderMatchId]);
    assert.equal(order.rows[0].tracking_code, "16341539811");
    assert.equal(order.rows[0].tracking_provider, "familymart");

    const tracking = await pool.query(
      `SELECT * FROM shipment_trackings WHERE order_id = $1 AND tracking_code = '16341539811'`, [orderMatchId]);
    ...
    const imported = await pool.query(
      `SELECT count(*)::int n FROM logistics_import_rows WHERE batch_id = $1 AND match_status = 'imported'`, [batchId]);
    assert.equal(imported.rows[0].n, dryRun.matchedRows);

    const batch = await pool.query(`SELECT status, confirmed_at FROM logistics_import_batches WHERE id = $1`, [batchId]);
    assert.equal(batch.rows[0].status, "confirmed");
    assert.ok(batch.rows[0].confirmed_at);

    const log = await pool.query(
      `SELECT * FROM shipment_tracking_run_logs WHERE store_id = $1 AND run_type = 'import_confirm' ORDER BY id DESC LIMIT 1`, [storeId]);
  ...
  test("idempotent re-run: same order + same code is imported again, no duplicate, no conflict", async () => {
    ...
    const rows = await pool.query(
      `SELECT id FROM logistics_import_rows WHERE batch_id = $1 AND tracking_code = '16341539811'`, [batchId]);
    assert.ok(rows.rows.length >= 1);
    const notFound = await pool.query(
      `SELECT id FROM logistics_import_rows WHERE batch_id = $1 AND match_status = 'not_found' LIMIT 1`, [batchId]);
    const ids = [...rows.rows.map((r) => r.id), notFound.rows[0].id];

    const res = await confirm(batchId, { rowIds: ids });
    const body = await res.json();
    assert.equal(res.status, 200);
    assert.equal(body.importedCount, rows.rows.length, "same-order same-code re-run counts as success");
    assert.ok(!body.rows.some((r) => r.errorCode === "TRACKING_CODE_CONFLICT"), "must not conflict on idempotent re-run");
    ...
    assert.equal(tracking.rows[0].is_active, true);
    assert.equal(tracking.rows[0].tracking_status, "pending", "status must not regress or change");

    const rowState = await pool.query(
      `SELECT match_status FROM logistics_import_rows WHERE id = $1`, [rows.rows[0].id]);
    assert.equal(rowState.rows[0].match_status, "imported");

    const log = await pool.query(
      `SELECT * FROM shipment_tracking_run_logs WHERE store_id = $1 AND run_type = 'import_confirm' ORDER BY id DESC LIMIT 1`, [storeId]);
    assert.equal(log.rows[0].success_count, rows.rows.length, "re-run counted in success_count");

    const exc = await pool.query(
      `SELECT error_code, message FROM shipment_tracking_exceptions WHERE import_batch_id = $1`, [batchId]);
    assert.ok(!exc.rows.some((e) => e.error_code === "TRACKING_CODE_CONFLICT"), "no conflict exception on re-run");
    for (const e of exc.rows) assert.ok(!/林模賢|0970000090/.test(e.message ?? ""), "exception message must not contain PII");
  });
  ...
  test("same order + different code → ORDER_ALREADY_HAS_TRACKING, no overwrite", async () => {
    ...
    const { batchId } = await (await famiUpload()).json();
    await pool.query(`UPDATE orders SET tracking_code = 'OTHERCODE999' WHERE id = $1`, [orderMatchId]);
    try {
      const res = await confirm(batchId, { confirmAllMatched: true });
      const body = await res.json();
      assert.equal(res.status, 200);
      assert.ok(body.rows.some((r) => r.status === "skipped" && r.errorCode === "ORDER_ALREADY_HAS_TRACKING"));

      const order = await pool.query(`SELECT tracking_code FROM orders WHERE id = $1`, [orderMatchId]);
      assert.equal(order.rows[0].tracking_code, "OTHERCODE999", "existing code must not be overwritten");

      const exc = await pool.query(
        `SELECT error_code FROM shipment_tracking_exceptions WHERE import_batch_id = $1`, [batchId]);
      assert.ok(exc.rows.some((e) => e.error_code === "ORDER_ALREADY_HAS_TRACKING"));
    } finally {
      await pool.query(
        `UPDATE orders SET tracking_code = '16341539811', tracking_provider = 'familymart' WHERE id = $1`, [orderMatchId]);
    }
  });
  ...
  test("different order + same code → TRACKING_CODE_CONFLICT", async () => {
    ...
    await pool.query(`UPDATE shipment_trackings SET order_id = $1 WHERE tracking_code = '16341539811'`, [orderOtherId]);
    await pool.query(`UPDATE orders SET tracking_code = NULL, tracking_provider = NULL WHERE id = $1`, [orderMatchId]);
    try {
      const { batchId } = await (await famiUpload()).json();
      const res = await confirm(batchId, { confirmAllMatched: true });
      const body = await res.json();
      assert.equal(res.status, 200);
      assert.ok(body.rows.some((r) => r.status === "skipped" && r.errorCode === "TRACKING_CODE_CONFLICT"));

      const order = await pool.query(`SELECT tracking_code FROM orders WHERE id = $1`, [orderMatchId]);
      assert.equal(order.rows[0].tracking_code, null, "conflicting code must not be written to the order");
      const trackingCount = await pool.query(
        `SELECT count(*)::int n FROM shipment_trackings WHERE tracking_code = '16341539811'`);
      ...
      const exc = await pool.query(
        `SELECT error_code FROM shipment_tracking_exceptions WHERE import_batch_id = $1`, [batchId]);
      assert.ok(exc.rows.some((e) => e.error_code === "TRACKING_CODE_CONFLICT"));
    } finally {
      await pool.query(`UPDATE shipment_trackings SET order_id = $1 WHERE tracking_code = '16341539811'`, [orderMatchId]);
      await pool.query(
        `UPDATE orders SET tracking_code = '16341539811', tracking_provider = 'familymart' WHERE id = $1`, [orderMatchId]);
    }
  });
  ...
  test("other user cannot confirm this store's batch", async () => {
    const { batchId } = await (await famiUpload()).json();
    const res = await confirm(batchId, { confirmAllMatched: true }, "someone-else");
