...
const { PUBLIC_ORDER_CREATED_RESPONSE_KEYS } =
  await import("../lib/publicOrderResponse.ts");
const { PUBLIC_TRACK_ORDER_RESPONSE_KEYS } =
  await import("../lib/publicTrackResponse.ts");
const { PUBLIC_CART_ITEM_RESPONSE_KEYS } =
  await import("../lib/publicCartItems.ts");
...
before(async () => {
  ...
  const [store] = await db
    ...
    .values({
      ...
      name: "__test_store_6eb__",
      slug: `test-store-6eb-${Date.now()}`,
    })
    .returning();
...
function assertPublicOrderCreatedResponse(data) {
  ...
  assert.ok(
    !("buyerName" in data),
    "public response must not expose buyerName",
  );
  assert.ok(
    !("storeSelectedBy" in data),
    "public response must not expose storeSelectedBy",
  );
  assert.ok(
    !("storeSelectedAt" in data),
    "public response must not expose storeSelectedAt",
  );
}
...
function assertPublicCartItemKeys(items, responseName) {
  ...
  for (const item of items) {
    ...
    assert.equal(
      Object.hasOwn(item, "profitSnapshot"),
      false,
...
describe("POST /p/:shareToken/orders — basic (no CVS)", () => {
  test("201 — creates order without CVS fields", async () => {
    ...
    assert.strictEqual(
      data.cvsStoreId,
      null,
      "cvsStoreId should be null when no CVS",
    );
  });
...
describe("POST /p/:shareToken/orders — storeSelectedAt", () => {
  ...
  test("storeSelectedAt is null when no CVS snapshot", async () => {
    ...
    const order = await readCreatedOrder(data.publicToken);
    assert.strictEqual(
      order.storeSelectedAt,
      null,
      "storeSelectedAt should be null without CVS",
    );
  });
});
...
describe("public cart responses — nested item privacy", () => {
  test("cart creation and tracking expose only the cart-item element allowlist", async () => {
    ...
    assert.strictEqual(
      status,
      201,
      `expected 201, got ${status}: ${JSON.stringify(created)}`,
    );
    ...
    assert.ok(
      ...
      "test fixture must store cart items",
    );
    assert.equal(
      Object.hasOwn(stored.items[0], "profitSnapshot"),
      true,
...
describe("GET /orders/track/:publicToken — privacy protection", () => {
  ...
  test("200 — returns order status with allowed fields", async () => {
    ...
    for (const forbidden of [
      ...
      assert.equal(
        Object.hasOwn(data, forbidden),
        false,
        `${forbidden} must never be public`,
      );
    }
  });
  ...
  test("does NOT return recipientPhone", async () => {
    ...
    assert.strictEqual(
      data.recipientPhone,
      undefined,
      "recipientPhone must not be returned",
    );
  });
  ...
  test("does NOT return recipientAddress", async () => {
    ...
    assert.strictEqual(
      ...
      undefined,
      "recipientAddress must not be returned",
    );
  });
  ...
  test("does NOT return paymentNote", async () => {
    ...
    assert.strictEqual(
      data.paymentNote,
      undefined,
      "paymentNote must not be returned",
    );
  });
  ...
  test("does NOT return paidAmount", async () => {
    ...
    assert.strictEqual(
      data.paidAmount,
      undefined,
      "paidAmount must not be returned",
    );
  });
  ...
  test("does NOT return buyerPhone (personal info)", async () => {
    ...
    assert.strictEqual(
      data.buyerPhone,
      undefined,
      "buyerPhone must not be returned",
    );
  });
  ...
  test("404 — unknown publicToken", async () => {
    const { status } = await req(
      "GET",
      "/orders/track/totally-unknown-public-token-xyz999",
    );
    assert.strictEqual(status, 404);
  });
});
