...
router.post(
  ...
  async (req: any, res: any) => {
    ...
    try {
      ...
      const eventCount = result.results.reduce(
        (sum, r) => sum + (r.insertedEventCount ?? 0),
        0,
      );
      return res.json({
        ok: true,
...
async function validateManualProviderRequest(
  ...
  if (!Array.isArray(rawIds) || rawIds.length === 0) {
    fail(
      res,
      400,
      "TRACKING_IDS_REQUIRED",
      "trackingIds must be a non-empty array",
    );
    return null;
  }
  ...
  if (trackingIds.some((n: number) => !Number.isInteger(n) || n <= 0)) {
    fail(
      ...
      "trackingIds must be positive integers",
    );
    return null;
  }
  ...
  if (rows.some((r) => r.storeId !== storeId)) {
    fail(
      res,
      400,
      "CROSS_STORE_TRACKING",
      "trackingIds 包含不屬於此店家的紀錄。",
    );
    return null;
  }
  ...
  async (req: any, res: any) => {
    ...
    if ((req.body ?? {}).dryRun === false) {
      return fail(
        ...
        400,
        "USE_COMMIT_ENDPOINT",
        "Manual provider write requires preview/commit flow.",
      );
    }
    ...
    try {
      ...
      if (
        ...
        err.message.startsWith("BATCH_SIZE_EXCEEDED")
      ) {
...
async function handle711Preview(req: any, res: any): Promise<void> {
  ...
  if (
    rows.some((r) => normalizeSevenElevenProvider(r.trackingProvider) !== "711")
  ) {
    fail(
      res,
      400,
  ...
  for (const id of trackingIds) {
    ...
    try {
      const raw = await trackSevenElevenShipment({
      ...
      if (!bridged.ok) {
        jobs.push({
          ...
          errorCode: bridged.errorCode,
          skippedReason: bridged.message,
          previewHash: null,
        ...
        jobs.push({
          ...
          wouldWriteEvents: bridged.events.length,
          duplicateEvents: 0,
          ...
          commitDisabled: true,
          pickupStoreName:
            (bridged.rawSummary?.pickupStoreName as string | null) ?? null,
          pickupDeadline:
            (bridged.rawSummary?.pickupDeadline as string | null) ?? null,
          eventCount: bridged.events.length,
          events: evts,
        });
      }
    } catch (err) {
  ...
  await Promise.all(
    jobs
      ...
      .map((j) =>
        saveManualQuerySnapshot(
          j.trackingId,
          j.latestStatusText ?? null,
          j.latestEventAt ?? null,
          j.normalizedStatus ?? null,
        ),
      ),
  );
...
router.post(
  ...
  async (req: any, res: any) => {
    ...
    const reqProvider =
      typeof reqBody.provider === "string" ? reqBody.provider.trim() : "";
    ...
    try {
      ...
      return res.json({
        ...
        jobs,
        message:
          "查詢完成：最新貨態摘要已更新，重整後仍保留。（完整物流事件未寫入）",
      });
    } catch (err) {
      if (
        err instanceof Error &&
...
router.post(
  ...
  async (req: any, res: any) => {
    ...
    const expectedLatestStatusText =
      body.expectedLatestStatusText !== undefined
        ? body.expectedLatestStatusText
        : null;
    const expectedLatestEventAt =
      body.expectedLatestEventAt !== undefined
        ? body.expectedLatestEventAt
        : null;
    ...
    if (!Number.isInteger(trackingId) || trackingId <= 0) {
      return fail(
        ...
        400,
        "INVALID_TRACKING_ID",
        "trackingId must be a positive integer",
      );
    }

    // trackingCode
    const trackingCode =
      typeof rawTrackingCode === "string" ? rawTrackingCode.trim() : "";
    if (!trackingCode) {
      return fail(
        res,
        400,
    ...
    if (confirmText !== COMMIT_CONFIRM_TEXT) {
      return fail(
        ...
        "confirmText 錯誤，必須填入 WRITE_TRACKING_EVENTS",
      );
    }
    ...
    if (
      !Number.isInteger(reqEventCount) ||
      reqEventCount < 0 ||
      reqEventCount !== tokenPayload.expectedEventCount
    ) {
      return fail(
        res,
        400,
        "EXPECTED_EVENT_COUNT_MISMATCH",
        "expectedEventCount 與預覽不符",
      );
    }
    const normReqStatus =
      expectedLatestStatusText === null
        ? null
        : String(expectedLatestStatusText);
    const normReqAt =
      expectedLatestEventAt === null ? null : String(expectedLatestEventAt);
    if (normReqStatus !== (tokenPayload.latestStatusText ?? null)) {
      return fail(
        res,
        400,
        "EXPECTED_LATEST_STATUS_MISMATCH",
        "expectedLatestStatusText 與預覽不符",
      );
    }
    if (normReqAt !== (tokenPayload.latestEventAt ?? null)) {
      return fail(
        ...
        "expectedLatestEventAt 與預覽不符",
      );
    }

    // load tracking row scoped by storeId
    let trackingRow:
      | { trackingCode: string; trackingProvider: string; isActive: boolean }
      | undefined;
    try {
      const rows = await db
        .select({
    ...
    if (
      !writeJob ||
      writeJob.status === "failed" ||
      writeJob.status === "skipped"
    ) {
      return fail(res, 500, "WRITE_FAILED", "寫入失敗，請稍後再試。");
    }
...
router.get(
  "/stores/:storeId/logistics/sync/status",
  requireAuth,
  async (req: any, res: any) => {
    const storeId = parseInt(req.params.storeId);
    if (isNaN(storeId))
