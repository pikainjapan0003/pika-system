...
const VALID_AGENT_MODE_SELLER = new Set(["self_hosted_webhook", "external_agent", "rule_worker"]);
const VALID_QUERY_FREQUENCY = new Set(["manual", "daily", "every_6_hours", "every_2_hours_high_tier"]);
const VALID_LOGISTICS = new Set(["seven_eleven", "family_mart", "home_delivery", "other", "webhook"]);
const VALID_QUERY_METHODS = new Set(["manual", "csv_import", "webhook", "scheduled"]);

const ALLOWED_PATCH_KEYS = new Set([
  "agentStatus", "agentMode", "enabledLogistics", "queryMethods", "queryFrequency",
  "notifyOnUnknown", "requireConfirmOnException", "requireConfirmOnReturned",
  "requireConfirmOnDelivered", "hideErrorDetailsFromBuyer",
  "webhookEnabled", "webhookUrl", "webhookSecret",
]);

const FORBIDDEN_PATCH_KEYS = new Set([
  "id", "storeId", "merchantId", "createdAt", "updatedAt",
  "lastRunAt", "lastTestRunAt", "webhookSecretHash",
]);
...
function toSafeSettings(row: typeof sellerAgentSettingsTable.$inferSelect): SafeSettings {
  return {
    id: row.id,
    storeId: row.storeId,
...
router.get("/stores/:storeId/agent/settings", requireAuth, async (req: any, res) => {
  try {
...
router.patch("/stores/:storeId/agent/settings", requireAuth, async (req: any, res) => {
  try {
    const storeId = parseInt(req.params.storeId, 10);
    if (isNaN(storeId)) return res.status(400).json({ error: "Invalid storeId" });
    ...
    for (const key of Object.keys(body)) {
      if (FORBIDDEN_PATCH_KEYS.has(key)) {
        return res.status(400).json({ error: `Field "${key}" is not patchable` });
      }
    ...
    if ("enabledLogistics" in body) {
      if (!Array.isArray(body.enabledLogistics)) {
        return res.status(400).json({ error: "enabledLogistics must be an array" });
      }
    ...
    if ("queryMethods" in body) {
      if (!Array.isArray(body.queryMethods)) {
        return res.status(400).json({ error: "queryMethods must be an array" });
      }
    ...
    if ("notifyOnUnknown" in body) {
      if (typeof body.notifyOnUnknown !== "boolean") {
        return res.status(400).json({ error: "notifyOnUnknown must be a boolean" });
      }
    ...
    if ("requireConfirmOnException" in body) {
      if (typeof body.requireConfirmOnException !== "boolean") {
        return res.status(400).json({ error: "requireConfirmOnException must be a boolean" });
      }
    ...
    if ("requireConfirmOnReturned" in body) {
      if (typeof body.requireConfirmOnReturned !== "boolean") {
        return res.status(400).json({ error: "requireConfirmOnReturned must be a boolean" });
      }
    ...
    if ("requireConfirmOnDelivered" in body) {
      if (typeof body.requireConfirmOnDelivered !== "boolean") {
        return res.status(400).json({ error: "requireConfirmOnDelivered must be a boolean" });
      }
    ...
    if ("hideErrorDetailsFromBuyer" in body) {
      if (typeof body.hideErrorDetailsFromBuyer !== "boolean") {
        return res.status(400).json({ error: "hideErrorDetailsFromBuyer must be a boolean" });
      }
    ...
    if ("webhookEnabled" in body) {
      if (typeof body.webhookEnabled !== "boolean") {
        return res.status(400).json({ error: "webhookEnabled must be a boolean" });
      }
    ...
    if ("webhookSecret" in body) {
      if (body.webhookSecret === null) {
        ...
        if (typeof body.webhookSecret !== "string" || body.webhookSecret.length < 16) {
          return res.status(400).json({ error: "webhookSecret must be a string of at least 16 characters" });
        }
        if (body.webhookSecret.length > 256) {
          return res.status(400).json({ error: "webhookSecret must not exceed 256 characters" });
        }
        // SHA-256 hash; never store or log the plaintext
        patch.webhookSecretHash = createHash("sha256").update(body.webhookSecret).digest("hex");
      }
    }
