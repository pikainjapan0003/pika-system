...
if (!integrationEnabled) {
  ...
  async function cleanupAll() {
    if (!storeMain?.id) {
      throw new Error(
        "STEP7D_E2E cleanup aborted: storeMain not seeded — refusing to run cleanup to prevent accidental data loss",
      );
    }
  ...
  describe("Flow A — Full happy path", () => {
    test("A-1: GET /orders/tracking-jobs returns seeded tracking for store main", async () => {
      ...
      assert.strictEqual(
        job.buyerName,
        undefined,
        "buyerName must not be in response",
      );
      assert.strictEqual(
        job.buyerPhone,
        undefined,
        "buyerPhone must not be in response",
      );
    });
    ...
    test("A-4: POST /run-log inserts agent_run_logs row with correct tokenId / storeId / merchantId", async () => {
      ...
      assert.strictEqual(
        row.storeId,
        storeMain.id,
        "storeId must match seeded store main",
      );
      assert.strictEqual(
        row.tokenId,
        tokenMain.id,
        "tokenId must match seeded token",
      );
      assert.strictEqual(
        row.merchantId,
        "STEP7D_E2E_merchant",
        "merchantId must match seeded value",
      );
    });
  });
  ...
  describe("Flow B — Cross-store isolation", () => {
    test("B-1: GET /orders/tracking-jobs with store A token does NOT return store B tracking", async () => {
      ...
      assert.strictEqual(status, 200);
      const hasStoreBTracking = data.jobs.some(
        (j) => j.trackingId === trackingB.id,
      );
      assert.strictEqual(
        hasStoreBTracking,
        false,
        "store B tracking must not appear in store A token results",
      );
    });
  ...
  describe("Flow E — Validation errors and auth", () => {
    ...
    test("E-3: Invalid tracking status filter → 400 invalid_tracking_status", async () => {
      const { status, data } = await req(
        "GET",
        "/orders/tracking-jobs?status=INVALID_STATUS",
        undefined,
        RAW_TOKEN_MAIN,
      );
  ...
  describe("Flow F — DB field verification", () => {
    test("F-1: POST /run-log — agent_run_logs.token_id equals seeded token id", async () => {
      ...
      assert.strictEqual(
        row.tokenId,
        tokenMain.id,
        "tokenId must match seeded token (not from body)",
      );
    });
  ...
  describe("Flow G — Cleanup guard (structural)", () => {
    test("G-1: cleanupAll() is a function and will run in after() hook", () => {
      assert.strictEqual(
        typeof cleanupAll,
        "function",
        "cleanupAll must be a function",
      );
      assert.ok(
        storeMain?.id,
        "storeMain must be seeded before cleanup guard passes",
      );
    });
  });
}
