...
router.get("/cvs/regions", async (req, res) => {
  const provider =
    typeof req.query.provider === "string" ? req.query.provider : "seven";

  try {
    const rows = await db
      .selectDistinct({
        city: cvsStoresTable.city,
        district: cvsStoresTable.district,
      })
      .from(cvsStoresTable)
      .where(
        and(
...
router.get("/cvs/stores", async (req, res) => {
  ...
  try {
    ...
    if (district)
      conditions.push(ilike(cvsStoresTable.district, `%${district}%`));
    ...
    if (rawQ) {
      ...
      rows = await db
        ...
        .orderBy(
          sql`(${cvsStoresTable.storeId} = ${rawQ}) DESC`,
          cvsStoresTable.storeName,
        )
        .limit(limit);
    } else {
      rows = await db
...
router.post("/cvs/711/import-from-emap", requireAuth, async (req: any, res) => {
  ...
  return res.status(403).json({
    error:
      "This operation is temporarily unavailable. Please contact the administrator.",
  });
  ...
  try {
    ...
    if (!resp.ok) {
      return res
        .status(502)
        .json({ error: "查詢 7-11 電子地圖失敗，請稍後再試" });
    }
...
function parseUpdateOrderCvsBody(
  body: any,
): { ok: true; data: UpdateOrderCvsData } | { ok: false; error: string } {
  ...
  return {
    ...
    data: {
      ...
      cvsStoreName:
        typeof body.cvsStoreName === "string" ? body.cvsStoreName : undefined,
      cvsStoreAddress:
        typeof body.cvsStoreAddress === "string"
          ? body.cvsStoreAddress
          : undefined,
