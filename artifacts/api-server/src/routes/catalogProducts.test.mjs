...
import {assertPhase3Database} from '../../../../scripts/product-database-test-db.mjs';
import * as schemas from '@workspace/api-zod';
assertPhase3Database(process.env.DATABASE_URL,process.env.PIKA_PHASE3_CONTAINER,process.env.PIKA_PHASE3_DISPOSABLE);
...
const {default:express}=await import('express');const {pool}=await import('@workspace/db');
const {default:catalog}=await import('./catalogProducts.ts');const {default:settings}=await import('./catalogPricingSettings.ts');
