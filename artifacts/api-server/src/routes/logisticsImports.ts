...
function parseMulter(req: any, res: any): Promise<void> {
  return new Promise((resolve, reject) => {
    multerUpload.single("file")(req, res, (err) => (err ? reject(err) : resolve()));
  });
}
 ...
 */
router.post("/stores/:storeId/logistics/imports/dry-run", requireAuth, async (req: any, res: any) => {
  ...
  if (provider !== "711" && provider !== "familymart")
    return fail(res, 400, "INVALID_PROVIDER", "provider must be 711 or familymart");

  if (!req.file || !req.file.buffer?.length)
    return fail(res, 400, "MISSING_FILE", "file is required (multipart field: file)");

  const fileName = req.file.originalname || "upload.xlsx";
  if (!fileName.toLowerCase().endsWith(".xlsx") || !XLSX_MIME_TYPES.has(req.file.mimetype))
    return fail(res, 400, "UNSUPPORTED_FILE_TYPE", "Only .xlsx files are accepted");

  const tmpPath = path.join(tmpdir(), `logistics-dryrun-${randomBytes(8).toString("hex")}.xlsx`);
  try {
    ...
    try {
      ...
      if (msg.startsWith("FORM_PARSE_FAILED"))
        return fail(res, 422, "REQUIRED_COLUMNS_MISSING", "Spreadsheet headers do not match the expected export format");
      return fail(res, 422, "PARSE_FAILED", "Could not parse the spreadsheet");
    }
    ...
    try {
      ...
      return fail(res, 500, "ORDERS_READ_FAILED", "Could not load orders for matching");
    }
    ...
    const batch = await db.transaction(async (tx) => {
      ...
      if (dryRun.rows.length) {
        await tx.insert(logisticsImportRowsTable).values(
          dryRun.rows.map((r) => {
            const sanitized = sanitizeImportRowForStorage(rowByNumber.get(r.rowNumber)!);
            return {
              batchId: created.id,
              ...
              externalOrderNo: rowByNumber.get(r.rowNumber)?.externalOrderNo ?? null,
    ...
    return res.json({ ok: true, provider, fileName, batchId: batch.id, dryRun });
  } catch {
 ...
 */
router.post("/stores/:storeId/logistics/imports/:batchId/confirm", requireAuth, async (req: any, res: any) => {
  const storeId = parseInt(req.params.storeId);
  const batchId = parseInt(req.params.batchId);
  if (isNaN(storeId) || isNaN(batchId)) return fail(res, 400, "INVALID_REQUEST", "Invalid storeId or batchId");
  if (!(await verifyStoreOwner(req, res, storeId))) return;

  const rowIds: number[] | undefined = Array.isArray(req.body?.rowIds) ? req.body.rowIds.map(Number) : undefined;
  const confirmAllMatched = req.body?.confirmAllMatched === true;
  if (!confirmAllMatched && (!rowIds || rowIds.length === 0))
    return fail(res, 400, "INVALID_CONFIRM_REQUEST", "Provide rowIds or confirmAllMatched=true");

  const [batch] = await db
    .select()
    .from(logisticsImportBatchesTable)
    .where(and(eq(logisticsImportBatchesTable.id, batchId), eq(logisticsImportBatchesTable.storeId, storeId)));
  if (!batch) return fail(res, 404, "BATCH_NOT_FOUND", "Batch not found for this store");
  if (batch.status !== "dry_run") return fail(res, 409, "BATCH_ALREADY_CONFIRMED", `Batch status is ${batch.status}`);
  ...
  const targetRows = await db
    ...
    .where(
      confirmAllMatched
        ? and(eq(logisticsImportRowsTable.batchId, batchId), eq(logisticsImportRowsTable.matchStatus, "matched"))
        : and(eq(logisticsImportRowsTable.batchId, batchId), inArray(logisticsImportRowsTable.id, rowIds!))
    );
  if (!targetRows.length) return fail(res, 400, "NO_ROWS_SELECTED", "No rows to confirm");

  type RowResult = { rowId: number; rowNumber: number; status: "imported" | "skipped"; errorCode: string | null };
  const results: RowResult[] = [];
  const exceptions: Array<typeof shipmentTrackingExceptionsTable.$inferInsert> = [];

  try {
    await db.transaction(async (tx) => {
      for (const row of targetRows) {
        const skip = (errorCode: string) => {
          results.push({ rowId: row.id, rowNumber: row.rowNumber, status: "skipped", errorCode });
          exceptions.push({
            storeId,
        ...
        if (row.matchStatus !== "matched") { skip("ROW_NOT_IMPORTABLE"); continue; }
        if (!row.matchedOrderId || !row.trackingCode) { skip("ORDER_NOT_FOUND"); continue; }

        const [order] = await tx.select().from(ordersTable).where(eq(ordersTable.id, row.matchedOrderId));
        if (!order || order.storeId !== storeId) { skip("ORDER_NOT_FOUND"); continue; }
        if (order.trackingCode && order.trackingCode !== row.trackingCode) { skip("ORDER_ALREADY_HAS_TRACKING"); continue; }

        const [dupTracking] = await tx
          .select({ id: shipmentTrackingsTable.id, orderId: shipmentTrackingsTable.orderId })
        ...
        if (dupTracking) {
          if (dupTracking.orderId === order.id) {
            ...
            await tx
              .update(ordersTable)
              .set({ trackingCode: row.trackingCode, trackingProvider: batch.provider })
              .where(eq(ordersTable.id, order.id));
            await tx
              .update(logisticsImportRowsTable)
              .set({ matchStatus: "imported" })
              .where(eq(logisticsImportRowsTable.id, row.id));
            results.push({ rowId: row.id, rowNumber: row.rowNumber, status: "imported", errorCode: null });
            continue;
          }
      ...
      const skippedIds = results.filter((r) => r.status === "skipped").map((r) => r.rowId);
      if (skippedIds.length) {
        await tx
          .update(logisticsImportRowsTable)
          .set({ matchStatus: "skipped" })
          .where(and(inArray(logisticsImportRowsTable.id, skippedIds), eq(logisticsImportRowsTable.matchStatus, "matched")));
      }
      ...
      if (exceptions.length) await tx.insert(shipmentTrackingExceptionsTable).values(exceptions);

      const successCount = results.filter((r) => r.status === "imported").length;
      const skippedCount = results.length - successCount;
      const errorCodes = [...new Set(results.filter((r) => r.errorCode).map((r) => r.errorCode))];
      await tx.insert(shipmentTrackingRunLogsTable).values({
        ...
        startedAt: new Date(),
        finishedAt: new Date(),
        status: skippedCount === 0 ? "success" : successCount === 0 ? "failed" : "partial",
...
const SKIPPED_MATCH_STATUSES = new Set(["skipped", "conflict", "not_found", "ambiguous", "needs_review"]);

/**
 * 匯入批次列表（store-scoped）。回傳安全欄位 + 由 rows matchStatus 推導的
 * success/skipped/failed/pending 計數。不回 rawRowJson / 個資。
 */
router.get("/stores/:storeId/logistics/import-batches", requireAuth, async (req: any, res: any) => {
  const storeId = parseInt(req.params.storeId);
  if (isNaN(storeId)) return fail(res, 400, "INVALID_STORE", "Invalid storeId");
  if (!(await verifyStoreOwner(req, res, storeId))) return;
  ...
  if (provider !== "all" && provider !== "711" && provider !== "familymart")
    return fail(res, 400, "INVALID_PROVIDER", "provider must be 711|familymart|all");
  const limit = Math.min(Math.max(parseInt(String(req.query.limit ?? "20")) || 20, 1), 50);

  const conditions = [eq(logisticsImportBatchesTable.storeId, storeId)];
  if (provider !== "all") conditions.push(eq(logisticsImportBatchesTable.provider, provider));
  ...
  try {
    ...
    const statusCounts = batchIds.length
      ? await db
          ...
          .groupBy(logisticsImportRowsTable.batchId, logisticsImportRowsTable.matchStatus)
      : [];

    const countsByBatch = new Map<number, { success: number; skipped: number; failed: number; pending: number }>();
    for (const { batchId, matchStatus, n } of statusCounts) {
      const c = countsByBatch.get(batchId) ?? { success: 0, skipped: 0, failed: 0, pending: 0 };
    ...
    const items = batches.map((b) => {
      const c = countsByBatch.get(b.id) ?? { success: 0, skipped: 0, failed: 0, pending: 0 };
      return {
        id: b.id,
        provider: b.provider,
        sourceType: "file_import",
    ...
    return fail(res, 500, "BATCHES_READ_FAILED", "Could not load import batches");
  }
});
...
router.get(
  ...
  async (req: any, res: any) => {
    ...
    try {
      ...
      if (status !== "all") rowConditions.push(eq(logisticsImportRowsTable.matchStatus, status));
      ...
      return fail(res, 500, "BATCH_ROWS_READ_FAILED", "Could not load batch rows");
    }
  },
);
