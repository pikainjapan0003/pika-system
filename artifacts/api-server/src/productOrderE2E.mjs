...
import {assertPhase6Database} from '../../../e2e/product-order-harness.mjs';
assert.equal(process.env.PIKA_PHASE6_E2E,'DB-BUILD-06');
assertPhase6Database(process.env.DATABASE_URL,process.env.PIKA_PHASE6_CONTAINER,process.env.PIKA_PHASE6_E2E);
const require=createRequire(import.meta.url),owner='phase6-owner-'+Date.now();
...
for(const name of ['stores','categories','catalogProducts','catalogPricingSettings','pricingV2','products','trips','catalogListings','catalogOrders','orders','public','customers','chartData','fixedCostSummary'])app.use('/api',(await import(`./routes/${name}.ts`)).default);
...
export const fixture={port:server.address().port,pid:process.pid,store,other,category,owner};export {pool};
console.log('PHASE6_READY='+JSON.stringify(fixture));
let closing=false;export async function stop(){if(closing)return;closing=true;await new Promise(r=>server.close(r));await pool.end();input.close();console.log('PHASE6_STOPPED='+process.pid);}
