...
import {assertPhase5Database} from '../../../e2e/product-listing-harness.mjs';
assert.equal(process.env.PIKA_PHASE5_E2E,'DB-BUILD-05');
assertPhase5Database(process.env.DATABASE_URL,process.env.PIKA_PHASE5_CONTAINER,process.env.PIKA_PHASE5_E2E);
const require=createRequire(import.meta.url),owner='phase5-owner-'+Date.now();
...
for(const name of ['stores','categories','catalogProducts','catalogPricingSettings','pricingV2','products','trips','catalogListings'])app.use('/api',(await import(`./routes/${name}.ts`)).default);
...
console.log('PHASE5_READY='+JSON.stringify({port:server.address().port,pid:process.pid,store,other,category,owner}));
let closing=false;async function stop(){if(closing)return;closing=true;await new Promise(r=>server.close(r));await pool.end();console.log('PHASE5_STOPPED='+process.pid);process.exit(0);}
