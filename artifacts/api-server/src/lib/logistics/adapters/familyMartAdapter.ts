...
const ENDPOINT = "https://ecfme.fme.com.tw/FMEDCFPWebV2_II/list.aspx/GetOrderDetail";
...
export async function queryFamilyMartTracking(
  ...
  const failure = (
    ...
    message: string,
    retryable: boolean,
  ): FamilyMartTrackingResult => ({ ok: false, provider, trackingCode, errorCode, message, retryable });

  // 全家店到店單號為純數字（實測 11 碼）；保守允許 8–20 碼數字。
  if (!/^\d{8,20}$/.test(trackingCode)) {
    return failure("INVALID_TRACKING_CODE", `Invalid tracking code format (len=${trackingCode.length})`, false);
  }
  ...
  try {
    res = await doFetch(ENDPOINT, {
      ...
      body: JSON.stringify({ EC_ORDER_NO: trackingCode, ORDER_NO: trackingCode, RCV_USER_NAME: null }),
  ...
  let inner: { ErrorCode?: string; ErrorMessage?: string; List?: FamiRawEvent[] };
  try {
    const text = await res.text();
    const outer = JSON.parse(text);
    if (typeof outer?.d !== "string") {
      return failure("REMOTE_CHANGED", "Missing 'd' wrapper in response", false);
    }
    inner = JSON.parse(outer.d);
  } catch {
    return failure("PARSER_FAILED", "Response is not the expected JSON shape", false);
  }

  if (typeof inner?.ErrorCode !== "string" || !Array.isArray(inner?.List ?? [])) {
    return failure("REMOTE_CHANGED", "Inner payload missing ErrorCode/List", false);
  }

  if (inner.ErrorCode !== "000") {
    // 實測查無資料：ErrorCode "999" + 查無訂單資料
    const msg = `ErrorCode=${inner.ErrorCode} ${inner.ErrorMessage ?? ""}`.trim();
...
function toEvent(raw: FamiRawEvent): TrackingEvent {
  ...
  return {
    ...
    occurredAt: occurredAt && /^\d{4}\/\d{2}\/\d{2}/.test(occurredAt) ? occurredAt : null,
...
export function normalizeStatus(statusText: string): NormalizedTrackingStatus {
  ...
  if (/(已完成寄件|完成寄件|前往物流中心|物流中心|配送中|轉運|運送)/.test(s)) return "in_transit";
