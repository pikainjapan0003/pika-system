...
export function nameMaskMatch(masked: string | null, full: string | null): boolean {
  ...
  return (first === "*" || f[0] === first) && (last === "*" || f[f.length - 1] === last);
}

/** Masked phone match: same length, visible digit positions must agree. */
export function phoneMaskMatch(masked: string | null, full: string | null): boolean {
  if (!masked || !full) return false;
...
const remask = (s: string | null, masker: (v: string) => string): string | null =>
  s == null ? null : s.includes("*") ? s : masker(s);

// ---------- matching ----------

type Verdict = Pick<RowMatchResult, "matchStatus" | "matchedOrderId" | "confidence" | "candidateCount" | "reasons" | "errorCode">;
...
function matchRow(
  ...
  if (!row.trackingCode) return { matchStatus: "invalid", errorCode: "MISSING_TRACKING_CODE", reasons };
  if (!row.storeName) return { matchStatus: "invalid", errorCode: "MISSING_STORE_NAME", reasons };
  ...
  let candidates = orders.filter((o) => {
    ...
    if (o.shippingMethod !== "convenience_store" && !o.cvsStoreName) return false;
    return true;
  });

  // store: exact normalized first; contains is candidate-only (→ needs_review)
  const storeExact = rowStore ? candidates.filter((o) => normalizeStoreName(o.cvsStoreName) === rowStore) : [];
  const storeContains = rowStore
    ? candidates.filter((o) => !storeExact.includes(o) && normalizeStoreName(o.cvsStoreName).includes(rowStore))
    : [];
  candidates = storeExact.length ? storeExact : storeContains;
  if (!candidates.length) return { matchStatus: "not_found", errorCode: "NO_STORE_MATCH", reasons };
  reasons.push(storeExact.length ? `store_exact_match:${rowStore}` : `store_contains_match:${rowStore}`);
  ...
  let nameMatched = candidates.filter((o) => nameMaskMatch(row.recipientName, nameOf(o)));
  ...
  if (!candidates.length) return { matchStatus: "not_found", errorCode: "NO_NAME_MATCH", reasons };
  reasons.push(nameReason);

  // phone: familymart mandatory, Taiwan mobile (09xxxxxxxx) only
  if (provider === "familymart") {
    const mobileCandidates = candidates.filter((o) => isTaiwanMobile(o.recipientPhone || o.buyerPhone));
    if (!mobileCandidates.length) return { matchStatus: "not_found", errorCode: "PHONE_NOT_MOBILE", reasons };
    candidates = mobileCandidates.filter((o) => phoneMaskMatch(row.recipientPhone, o.recipientPhone || o.buyerPhone));
    if (!candidates.length) return { matchStatus: "not_found", errorCode: "NO_PHONE_MATCH", reasons };
    reasons.push("phone_mask_match");
  }

  if (candidates.length > 1)
    return { matchStatus: "ambiguous", errorCode: "MULTIPLE_CANDIDATES", candidateCount: candidates.length, reasons };

  const order = candidates[0];
  const trackingOwner = usedTracking.get(row.trackingCode);
  if (trackingOwner != null && trackingOwner !== order.id)
    return { matchStatus: "conflict", errorCode: "TRACKING_CODE_CONFLICT", reasons };
  for (const [code, oid] of usedTracking) {
    if (oid === order.id && code !== row.trackingCode)
      return { matchStatus: "conflict", errorCode: "ORDER_ALREADY_MATCHED", reasons };
  }
  usedTracking.set(row.trackingCode, order.id);
  reasons.push("unique_candidate");

  const containsOnly = reasons.some((r) => r.startsWith("store_contains_match"));
  const weakName = reasons.includes("name_not_verified");
  const matchStatus = containsOnly ? "needs_review" : "matched";
  const confidence =
    (containsOnly ? 70 : 95) + (provider === "familymart" && !weakName ? 5 : 0) - (weakName ? 15 : 0);
  return { matchStatus, matchedOrderId: order.id, confidence, reasons, errorCode: null };
}
 ...
 */
export function matchLogisticsImportRows(sheet: ParsedSpreadsheet, orders: CandidateOrder[]): DryRunReport {
  ...
  const count = (s: RowMatchResult["matchStatus"]) => rows.filter((r) => r.matchStatus === s).length;
  return {
