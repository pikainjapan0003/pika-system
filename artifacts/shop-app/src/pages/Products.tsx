      ...
      <div className="px-5 py-5">
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl p-10 border border-border text-center">
            ...
            <p className="text-foreground font-medium">還沒有商品</p>
            <p className="text-muted-foreground text-sm mt-1">新增第一個商品開始接單</p>
            <button
              onClick={() => setLocation("/products/new")}
              className="mt-4 h-10 px-6 bg-primary text-white text-sm font-semibold rounded-xl"
            >
              新增商品
            </button>
          </div>
            ...
            {products.map((p) => (
              <div key={p.id} className="bg-white rounded-2xl border border-border overflow-hidden">
                ...
                <div className="border-t border-border flex">
                  ...
                  >
                    {copiedId === p.id ? "已複製連結" : "複製分享連結"}
                  </button>
                  <button
                    onClick={() => setLocation(`/products/${p.id}/edit`)}
                    className="flex-1 py-3 text-xs text-foreground font-medium text-center border-r border-border"
                  >
                    編輯
                  </button>
                  <button
                    onClick={() => handleDelete(p.id)}
                    className="flex-1 py-3 text-xs text-destructive font-medium text-center"
                  >
                    刪除
                  </button>
                </div>
