...
export default function CustomerDetailPage({
  ...
  const [cvsDraft, setCvsDraft] = useState({
    id: "",
    name: "",
    address: "",
    phone: "",
  ...
  const load = async () => {
    ...
    const response = await fetch(
      `/api/stores/${store.id}/customers/${customerId}`,
      {
            ...
            <section className="space-y-3 rounded-2xl border border-border bg-white p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  value={cvsDraft.id}
                  onChange={(event) =>
                    setCvsDraft({ ...cvsDraft, id: event.target.value })
                  }
                />
                <input
                  className={inputClass}
                  ...
                  value={cvsDraft.name}
                  onChange={(event) =>
                    setCvsDraft({ ...cvsDraft, name: event.target.value })
                  }
                />
              </div>
              <input
                className={inputClass}
                placeholder="門市地址"
                value={cvsDraft.address}
                onChange={(event) =>
                  setCvsDraft({ ...cvsDraft, address: event.target.value })
                }
              />
              <input
                className={inputClass}
                placeholder="門市電話"
                value={cvsDraft.phone}
                onChange={(event) =>
                  setCvsDraft({ ...cvsDraft, phone: event.target.value })
                }
              />
                ...
                >
                  ...
                  >
                    ...
                    {order.profit.amountTwd === null
                      ? ""
                      : `：NT$${order.profit.amountTwd}${order.profit.scope === "unit" ? "／件" : ""}`}
                  </p>
                </article>
              ))}
              {orders.length === 0 && (
