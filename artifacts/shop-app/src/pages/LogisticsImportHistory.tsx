...
const STATUS_REASON_TEXTS: Record<
  string,
  { reason: string; description: string }
> = {
  imported: {
    reason: "已成功匯入",
    description: "這列資料已成功配對並建立物流追蹤資料。",
  },
  success: {
    reason: "已成功匯入",
    description: "這列資料已成功配對並建立物流追蹤資料。",
  ...
  needs_review: {
    reason: "需人工確認",
    description: "比對結果不夠確定，需要人工確認後再匯入。",
  },
  ambiguous: {
    reason: "可能對應多筆訂單",
    description: "這列資料可能符合多筆訂單，系統無法自動判斷要匯入哪一筆。",
  ...
  invalid: {
    reason: "資料不完整",
    description: "這列缺少必要資料（例如貨號），無法匯入。",
  },
  skipped: { reason: "已略過", description: "這列資料不需要再次匯入。" },
};

function getRowReasonText(row: ImportRow): {
  reason: string;
  description: string;
} {
  if (row.errorCode && ROW_REASON_TEXTS[row.errorCode])
    return ROW_REASON_TEXTS[row.errorCode];
  if (STATUS_REASON_TEXTS[row.status]) return STATUS_REASON_TEXTS[row.status];
  if (row.errorCode)
    return {
      reason: "其他匯入異常",
      description: "這列資料未完成匯入，請查看技術代碼或人工確認。",
    };
...
function matchesRowSearch(row: ImportRow, query: string): boolean {
  ...
  if (row.trackingCode && row.trackingCode.toLowerCase().includes(q))
    return true;
...
function filterRows(
  rows: ImportRow[],
  filter: "all" | RowResult,
  query: string,
): ImportRow[] {
  return rows.filter(
    (row) =>
      (filter === "all" || getRowResult(row) === filter) &&
...
export default function LogisticsImportHistoryPage() {
  ...
  const [rowFilterByBatch, setRowFilterByBatch] = useState<
    Record<number, "all" | RowResult>
  >({});
  const [rowSearchByBatch, setRowSearchByBatch] = useState<
  ...
  const fetchBatches = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/stores/${storeId}/logistics/import-batches?limit=20&provider=all`,
        {
          credentials: "include",
  ...
  const toggleRows = async (batchId: number) => {
    ...
    try {
      ...
      setRowsByBatch((prev) => ({
        ...prev,
        [batchId]: { state: "loaded", rows: body.rows ?? [] },
      }));
    } catch {
      setRowsByBatch((prev) => ({ ...prev, [batchId]: { state: "error" } }));
    }
              ...
              >
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-xs font-bold text-foreground">
                    {getProviderShortName(batch.provider) ?? "其他物流"}{" "}
                    {SOURCE_LABELS[batch.sourceType] ?? "Excel"}
                  </span>
                ...
                <div className="text-xs text-foreground space-y-0.5">
                  ...
                  <p className="truncate" title={batch.fileName}>
                    檔案：{shortFileName(batch.fileName)}
                  </p>
                </div>
                          ...
                          <>
                            {pageRows.map((row) => {
                              ...
                              const { reason, description } =
                                getRowReasonText(row);
                              return (
                                <div
                                  key={row.id}
                                  className="text-xs text-foreground space-y-0.5 border-b border-border/50 pb-2 last:border-b-0"
                                >
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-medium">
                                      Excel 第 {row.rowNumber} 列
                                    </span>
                                    <span
                                      className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${RESULT_BADGE[result]}`}
                                    >
                                      {RESULT_LABELS[result]}
                                    </span>
                                  </div>
                                  <p className="break-all">
                                    貨號：{row.trackingCode || "未提供"}
                                  </p>
                                  <p>
                                    訂單：
                                    {row.orderId != null
