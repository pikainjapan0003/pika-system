...
function isPickupMethodEnabled(method: string, settings: any): boolean {
  if (method.startsWith("7-11") || method.startsWith("全家"))
    return settings?.shippingCvsEnabled !== false;
  if (method === "黑貓宅急便")
    return settings?.shippingBlackCatEnabled !== false;
...
export default function PublicOrderPage({ shareToken }: Props) {
  ...
  const [submittedCvsStore, setSubmittedCvsStore] = useState<CvsStore | null>(
    null,
  );
  ...
  const cartJustAddedTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );

  const specs: Spec[] = (product?.specs as Spec[]) ?? [];
  const orderDeadlineAt = product?.orderDeadlineAt
  ...
  useEffect(() => {
    if (pickupMethod && !isPickupMethodEnabled(pickupMethod, product))
      setPickupMethod("");
  ...
  useEffect(() => {
    ...
    try {
      const rawDraft = sessionStorage.getItem(
        `public_order_draft_${shareToken}`,
      );
      if (rawDraft) {
        ...
        if (typeof draft.quantity === "number" && draft.quantity >= 1)
          setQuantity(draft.quantity);
        if (draft.specValues && typeof draft.specValues === "object")
          setSpecValues(draft.specValues);
      }
    } catch {}
  }, [shareToken]);
  ...
  const isOrderClosed =
    ...
    Number.isFinite(orderDeadlineAt.getTime()) &&
    now >= orderDeadlineAt;
  ...
  const handleSelectStore = () => {
    try {
      ...
      sessionStorage.setItem(
        `public_order_draft_${shareToken}`,
        JSON.stringify({ buyerName, buyerPhone, notes, quantity, specValues }),
      );
    } catch {}
    const basePath =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const recipientNamePayload = sameAsBuyer
      ? buyerName.trim()
      : recipientNameInput.trim();
    const recipientPhonePayload = sameAsBuyer
      ? buyerPhone.trim()
      : recipientPhoneInput.trim();
    if (!sameAsBuyer) {
      if (!recipientNamePayload) {
        setFormError("請輸入收件人");
        return;
      }
      if (!recipientPhonePayload) {
        setFormError("請輸入收件電話");
        return;
      }
    }
    ...
    try {
      ...
      if (needsCvsStore) {
        ...
        try {
          localStorage.removeItem(`cvs711_method_${shareToken}`);
        } catch {}
      }
      try {
        sessionStorage.removeItem(`public_order_draft_${shareToken}`);
      } catch {}
    } catch (err: any) {
      const reason =
        err?.data?.message || err?.data?.error || "網路或系統暫時沒有回應。";
      setFormError(
              ...
              <div className="flex flex-wrap gap-2">
                {product.storageTemp && (
                  <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-muted text-xs text-foreground">
                    溫層：
                    {product.storageTemp === "ambient"
                      ? "常溫"
                      : product.storageTemp === "chilled"
                        ? "冷藏"
                        : "冷凍"}
                  </span>
                )}
            ...
            <div className="flex flex-wrap gap-2">
              {spec.values.map((val) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setSpecValues({ ...specValues, [spec.name]: val })
                  }
                ...
                <div key={m}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                      ...
                      {/* Fee — mobile only */}
                      <span
                        className={`sm:hidden ml-auto text-sm font-semibold shrink-0 ${isSelected ? "text-primary" : "text-muted-foreground"}`}
                      >
                    ...
                    {/* Fee — desktop only */}
                    <span
                      className={`hidden sm:block text-base font-semibold shrink-0 ml-3 ${isSelected ? "text-primary" : "text-muted-foreground"}`}
                    >
                      {getShippingFeeLabel(m)}
                    </span>
                  </button>
