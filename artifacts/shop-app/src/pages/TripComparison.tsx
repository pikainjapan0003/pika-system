    ...
    (async () => {
      try {
        ...
        const response = await fetch(
          `/api/stores/${store.id}/trips/${tripId}/fixed-cost-comparison`,
          {
            ...
            <table className="w-full min-w-[560px] text-left text-sm">
              <thead>
                <tr className="border-b border-border text-xs text-muted-foreground">
                  ...
                  <th className="py-2">預估</th>
                  <th className="py-2">實際</th>
                  <th className="py-2">差異</th>
              ...
              <tbody>
                {rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="py-3">
                      {row.estimatedTwd == null
                        ? "未發生"
                        : `NT$${row.estimatedTwd}`}
                    </td>
                    <td className="py-3">
                      {row.actualTwd == null ? "預算外" : `NT$${row.actualTwd}`}
                    </td>
                    <td className="py-3">
