...
mock.module("wouter", { namedExports: { useLocation: () => ["/trips/1/comparison", () => undefined] } });
...
const { default: TripComparisonPage } = await import("../pages/TripComparison.tsx");
const payload = {
  status: "ready",
  rows: [
    { key: "category:1", label: "人事費用", state: "matched", estimatedTwd: "100.00", actualTwd: "80.00", variance: { difference: "-20.00", percent: "-0.20", direction: "favorable" } },
    { key: "category:2", label: "機票費用", state: "estimate-only", estimatedTwd: "50.00", actualTwd: null, variance: null },
    { key: "category:3", label: "自訂", state: "actual-only", estimatedTwd: null, actualTwd: "10.00", variance: null },
  ],
};
...
async function renderPage(responseBody = payload) {
  ...
  for (let i = 0; i < 20 && !container.textContent?.includes("人事費用"); i++) await new Promise((resolve) => setTimeout(resolve, 0));
...
test("comparison renders favorable and unfavorable direction labels", async () => {
  const { container, root } = await renderPage({ ...payload, rows: [{ ...payload.rows[0], variance: { ...payload.rows[0].variance, direction: "unfavorable" } }] });
