...
export const fakeSkills = ["S-01", "S-04"].map((skillKey) => ({
  skillKey,
  enabled: true,
  configured: true,
  highRisk: false,
...
export function readMock(path, { skills = fakeSkills } = {}) {
  if (path === "/api/me/store") return fakeStore;
  if (/^\/api\/stores\/\d+\/skills$/.test(path))
    return { catalogVersion: 1, skills };
  if (/\/stats$/.test(path))
    return {
      totalOrders: 0,
      totalRevenue: 0,
  ...
  if (/profit-summary/.test(path))
    return {
      capturedProfitSubtotalDisplayTwd: "0.00",
      pendingOrderCount: 0,
      totalRevenue: 0,
      totalCost: 0,
      totalProfit: 0,
...
export async function mockCalculator(
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (request.method() === "GET") {
      ...
      return route.fulfill({ json: readMock(path, { skills }) });
    }
