...
assertPhase5Database(identity.url,identity.id,'DB-BUILD-05');mkdirSync(evidence,{recursive:true});
const env={...process.env,DATABASE_URL:identity.url,DATABASE_SSLMODE:'disable',PGSSLMODE:'disable',PIKA_PHASE5_E2E:'DB-BUILD-05',PIKA_PHASE5_CONTAINER:identity.id};
delete env.PIKA_ORIGINAL_FOCUS_PROBE;
const apiLog=createWriteStream(path.join(evidence,'api.log'));const api=spawn(process.execPath,['--experimental-test-module-mocks','--import','tsx/esm','src/productListingE2E.mjs'],{cwd:path.join(root,'artifacts/api-server'),env,windowsHide:true,stdio:['pipe','pipe','pipe']});let apiExit;api.on('exit',code=>{apiExit=code;});api.stderr.pipe(apiLog,{end:false});let server,fixture,exitCode=1,testPid=null;
try{
 fixture=await new Promise((resolve,reject)=>{let buffer='';const timeout=setTimeout(()=>reject(Error('API startup timeout')),120000);api.once('error',reject);api.once('exit',code=>{clearTimeout(timeout);reject(Error('API exited '+code));});api.stdout.on('data',chunk=>{apiLog.write(chunk);buffer+=chunk.toString();const match=buffer.match(/PHASE5_READY=(\{[^\n]+\})/);if(match){clearTimeout(timeout);resolve(JSON.parse(match[1]));}});});
 ...
 const require=createRequire(import.meta.url);const tests=spawn(process.execPath,[require.resolve('@playwright/test/cli'),'test','--config',path.join(root,'e2e/product-listing.config.mjs'),...(grep?['--grep',grep]:[])],{cwd:root,env:{...env,E2E_BASE_URL:`http://127.0.0.1:${port}`,PIKA_PHASE5_EVIDENCE:evidence,PIKA_PHASE5_FIXTURES:JSON.stringify(fixture),PIKA_PHASE4_FIXTURES:JSON.stringify(fixture),PIKA_PHASE4_EVIDENCE:evidence},windowsHide:true,stdio:'inherit'});testPid=tests.pid;exitCode=await new Promise((resolve,reject)=>{tests.once('error',reject);tests.once('exit',resolve);});
}finally{
