...
export default defineConfig({testDir:'.',testMatch:['product-order.spec.mjs'],workers:1,fullyParallel:false,retries:0,timeout:180000,expect:{timeout:15000},outputDir:process.env.PIKA_PHASE6_BROWSER_EVIDENCE+'/test-results',reporter:[['line'],['junit',{outputFile:process.env.PIKA_PHASE6_BROWSER_EVIDENCE+'/e2e.junit.xml'}]],use:{baseURL:url.href,browserName:'chromium',headless:true,viewport:{width:390,height:844},screenshot:'only-on-failure',trace:'retain-on-failure',launchOptions:{args:['--disable-dev-shm-usage']}}});
