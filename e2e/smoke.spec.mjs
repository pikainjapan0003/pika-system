...
test.skip(process.env.CI !== "true", "BATCH-6 規格：Playwright 冒煙僅在 CI Linux 執行");

test("公開商品頁可開啟並顯示商品", async ({ page }) => {
  await page.goto("/p/ci-smoke-product");
  await expect(page.getByText("CI 冒煙測試商品", { exact: true })).toBeVisible();
});
...
test("未登入讀取後台訂單 API 會回 401", async ({ request }) => {
  const response = await request.get("http://127.0.0.1:8080/api/stores/1/orders");
