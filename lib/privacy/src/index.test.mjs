...
test("empty values return an empty string without throwing", () => {
  const maskers = [maskName, maskNameStrict, maskPhone, maskAddress, maskEmail, maskLineId];
...
test("strict public name masking never reveals the final character", () => {
  assert.equal(maskNameStrict("陳明"), "陳○");
  assert.equal(maskNameStrict("王"), "○");
  assert.equal(maskNameStrict("王小明"), "王○○");
  assert.equal(maskNameStrict("歐陽小明"), "歐○○○");
  assert.equal(maskNameStrict(""), "");
});
