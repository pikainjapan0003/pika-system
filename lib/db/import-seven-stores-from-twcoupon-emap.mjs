...
const urlArg =
  getArg("--url") ??
  "https://twcoupon.com/brandshopcity-7_11-%e9%80%a3%e6%b1%9f%e7%b8%a3-%e9%9b%bb%e8%a9%b1-%e5%9c%b0%e5%9d%80.html";
...
const limitCities =
  getArg("--limit-cities") != null
    ? parseInt(getArg("--limit-cities"), 10)
    : Infinity;
const skipCitiesArg = getArg("--skip-cities");
const onlyCitiesArg = getArg("--only-cities");
const skipCities = skipCitiesArg
  ? skipCitiesArg.split(",").map((s) => s.trim())
  : [];
const onlyCities = onlyCitiesArg
  ? onlyCitiesArg.split(",").map((s) => s.trim())
  : [];

const TWCOUPON_BASE = "https://twcoupon.com";
const MAIN_PAGE_URL =
  "https://twcoupon.com/brandshop-7_11-%E9%9B%BB%E8%A9%B1-%E5%9C%B0%E5%9D%80.html";

// Progress / report file paths (next to script, ignored by git)
const ALL_CITIES_PROGRESS_FILE = path.join(
  __dir,
  "twcoupon-emap-all-cities-progress.json",
);
const ALL_CITIES_REPORT_FILE = path.join(
  __dir,
  "twcoupon-emap-all-cities-report.json",
);
...
function isMainPageUrl(url) {
  return (
    url.includes("brandshop-7_11-") &&
    !url.includes("brandshopcity") &&
    !url.includes("brandshoparea")
  );
}
...
function parseStoresFromHtml(html, sourceUrl) {
  ...
  while ((m = blockRe.exec(html)) !== null) {
    stores.push({
      storeName: m[1].trim(),
      storeAddress: m[2].trim(),
      storePhone: m[3].trim() || null,
      sourceUrl,
    });
  }
  return stores;
...
function parseEmapCandidates(xml) {
  ...
  while ((m = re.exec(xml)) !== null) {
    ...
    let city = null,
      district = null;
    if (address) {
      const cm = address.match(/^(.{2,4}?[市縣])/);
      if (cm) city = cm[1];
      const dm = address.match(/[市縣](.{2,4}[區鄉鎮市])/);
      if (dm) district = dm[1];
    }
...
function makeUnverifiedRow(tw) {
  let city = null,
    district = null;
  if (tw.storeAddress) {
    const cm = tw.storeAddress.match(/^(.{2,4}?[市縣])/);
    if (cm) city = cm[1];
    const dm = tw.storeAddress.match(/[市縣](.{2,4}[區鄉鎮市])/);
    if (dm) district = dm[1];
  }
  return {
...
async function collectStoresFromCityPage(cityUrl) {
  ...
  if (!noSubPages) {
    ...
    for (const areaUrl of areaLinks) {
      ...
      try {
        ...
        const areaName = decodeURIComponent(
          areaUrl.split("brandshoparea-7_11-")[1]?.split("-電話")[0] ?? areaUrl,
        );
        console.log(`    [area] ${areaName}：${areaStores.length} 筆`);
        allStores.push(...areaStores);
      } catch (err) {
        console.log(
          `    [area] 失敗 ${decodeURIComponent(areaUrl)}：${err.message}`,
        );
      }
    }
  }
  ...
  return {
    stores: deduped,
    cityInlineCount: inlineStores.length,
    areaPageCount,
    rawCount: raw,
...
async function verifyAndUpsert(stores, cityName) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    process.stdout.write(
      `  [${i + 1}/${total}] 「${tw.storeName}」(kw:${keyword}) ... `,
    );
    ...
    if (verifyEmap && !dryRun) {
      try {
        ...
        if (candidates.length === 0) {
          ...
          let bestScore = addressSimilarity(
            tw.storeAddress,
            candidates[0].storeAddress,
          );
          ...
          stats.multipleCandidatesList.push({
            ...
            candidates: candidates.map(
              (c) => `${c.poiId} ${c.storeName} ${c.storeAddress}`,
            ),
            chosen: best.poiId,
          });
          dbRow = makeVerifiedRow(best);
          process.stdout.write(
            `verified(${candidates.length}cand,${best.poiId})`,
          );
        }
      } catch (err) {
        stats.emapFailed++;
...
function printCityStats(cityName, collResult, verResult, dbTotal) {
  console.log(`\n  ── ${cityName} 統計 ─────────────────────────────────`);
  console.log(
    `  twcoupon：${collResult.dedupedCount} 筆（raw=${collResult.rawCount}）`,
  );
  console.log(
    `  verified：${verResult.emapVerified}　unverified：${verResult.emapUnverified}　multi-cand：${verResult.emapMultiCandidates}　emap失敗：${verResult.emapFailed}`,
  );
  console.log(
    `  DB 新增：${verResult.inserted}　更新：${verResult.updated}　失敗：${verResult.dbFailed}`,
  );
  if (dbTotal > 0) console.log(`  DB 7-11 總數：${dbTotal}`);

  if (verResult.unverifiedList.length > 0) {
    console.log(`  [unverified]`);
    verResult.unverifiedList.forEach((s) =>
      console.log(`    - ${s.storeName}（kw:${s.keyword}）`),
    );
  }
...
function loadProgress() {
  ...
  try {
    return JSON.parse(fs.readFileSync(ALL_CITIES_PROGRESS_FILE, "utf-8"));
  } catch {
    return null;
  }
}
...
async function runAllCities() {
  ...
  if (onlyCities.length > 0) {
    cityUrls = cityUrls.filter((u) =>
      onlyCities.includes(extractCityNameFromUrl(u)),
    );
    if (!listCities)
      console.log(
        `--only-cities：篩選後 ${cityUrls.length} 個縣市（${onlyCities.join(",")}）`,
      );
  }
  if (skipCities.length > 0) {
    const before = cityUrls.length;
    cityUrls = cityUrls.filter(
      (u) => !skipCities.includes(extractCityNameFromUrl(u)),
    );
    if (!listCities)
      console.log(
        `--skip-cities：略過 ${before - cityUrls.length} 個，剩 ${cityUrls.length} 個`,
      );
  }
  ...
  if (resumeMode && progress) {
    ...
    const before = cityUrls.length;
    cityUrls = cityUrls.filter(
      (u) => !completed.has(extractCityNameFromUrl(u)),
    );
    if (!listCities)
      console.log(
        `--resume：已完成 ${completed.size} 個，略過後剩 ${cityUrls.length} 個`,
      );
  } else if (!resumeMode) {
  ...
  if (limitCities < Infinity) {
    ...
    if (!listCities)
      console.log(
        `--limit-cities ${limitCities}：本次處理 ${cityUrls.length} 個`,
      );
  }
  ...
  console.log(
    `delay：${delayMs}ms　EmapSDK：${verifyEmap ? "開啟" : "關閉"}　sub-pages：${noSubPages ? "關閉" : "開啟"}`,
  );
  ...
  const completedCities = progress?.completedCities
  ...
  for (let ci = 0; ci < cityUrls.length; ci++) {
    ...
    try {
      ...
      const storesToProcess =
        limit < Infinity
          ? collResult.stores.slice(0, limit)
          : collResult.stores;
  ...
  console.log("═".repeat(60));
  console.log(
    `處理縣市：   ${globalStats.processedCities} / ${globalStats.totalCities}`,
  );
  ...
  if (failedCities.length > 0)
    console.log(`失敗縣市：   ${failedCities.join(", ")}`);
...
async function runSingleCity() {
  ...
  if (isMainPageUrl(urlArg)) {
    ...
    const pocTarget = cityLinks.filter(
      (u) => u.includes("%e9%80%a3%e6%b1%9f") || u.includes("連江"),
    );
    cityPageUrls =
      pocTarget.length > 0 ? [pocTarget[0]] : [cityLinks[cityLinks.length - 1]];
  ...
  const storesToProcess =
    limit < Infinity ? allStores.slice(0, limit) : allStores;

  console.log(
    `\ntwcoupon 總計：${allStores.length} 筆，本次處理：${storesToProcess.length} 筆`,
  );
  console.log(
    `EmapSDK 驗證：${verifyEmap ? "開啟" : "關閉"}　delay：${delayMs}ms`,
  );
  if (dryRun) console.log("[DRY-RUN] 不會寫入 DB");
  console.log("");
  ...
  if (stats.unverifiedList.length > 0) {
    ...
    stats.unverifiedList.forEach((s, i) =>
      console.log(
        `  ${i + 1}. ${s.storeName}（kw:${s.keyword}）${s.storeAddress}`,
      ),
    );
  }
  if (stats.multipleCandidatesList.length > 0) {
    ...
    stats.multipleCandidatesList.forEach((s, i) => {
      console.log(
        `  ${i + 1}. ${s.storeName}（kw:${s.keyword}）→ chose ${s.chosen}`,
      );
  ...
  if (!dryRun) {
    fs.writeFileSync(
      path.join(reportDir, "twcoupon-emap-poc-report.json"),
      JSON.stringify(
        {
          sourceUrl: urlArg,
