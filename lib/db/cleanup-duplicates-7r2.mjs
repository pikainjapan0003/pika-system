...
async function main() {
  ...
  for (const sid of candidates) {
    ...
    const [cand] = await q(
      `SELECT ${COLS} FROM cvs_stores WHERE store_id=$1 AND provider='seven'`,
      [sid],
    );
    if (!cand) {
      console.log(`[${sid}] 找不到\n`);
    ...
    for (const row of dupes) {
      const tag = candidates.includes(String(row.store_id))
        ? "【停用】"
        : "【保留】";
  ...
  const [{ before }] = await q(
    `SELECT COUNT(*) AS before FROM cvs_stores WHERE provider='seven' AND is_active=true`,
  );
  const [{ before_inactive }] = await q(
    `SELECT COUNT(*) AS before_inactive FROM cvs_stores WHERE provider='seven' AND is_active=false`,
  );
  console.log(`\n=== PHASE 2: UPDATE 前 ===`);
  ...
  for (const r of result.rows)
    console.log(`    store_id=${r.store_id}  ${r.store_name}`);

  // === PHASE 4: post-checks ===
  console.log(`\n=== PHASE 4: UPDATE 後驗證 ===`);

  const [{ after }] = await q(
    `SELECT COUNT(*) AS after FROM cvs_stores WHERE provider='seven' AND is_active=true`,
  );
  const [{ after_inactive }] = await q(
    `SELECT COUNT(*) AS after_inactive FROM cvs_stores WHERE provider='seven' AND is_active=false`,
  );
  console.log(
    `  active   : ${after}  (before=${before}, diff=${parseInt(before) - parseInt(after)})`,
  );
  console.log(
    `  inactive : ${after_inactive}  (before=${before_inactive}, diff=${parseInt(after_inactive) - parseInt(before_inactive)})`,
  );
  ...
  if (addrDupes.length)
    addrDupes.forEach((r) =>
      console.log(`    ${r.store_name} / ${r.store_address}`),
    );
  ...
  if (cityAnomaly.length)
    cityAnomaly.forEach((r) => console.log(`    ${r.city} (${r.cnt})`));
