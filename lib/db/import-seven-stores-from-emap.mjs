...
if (!keywordsFile) {
  console.error(
    "Usage: node import-seven-stores-from-emap.mjs --file <keywords-file> [--delay <ms>] [--limit <n>] [--resume] [--dry-run]",
  );
  process.exit(1);
}
    ...
    JSON.stringify(
      {
        file: absFile,
        total: stats.total,
        processed: stats.processedThisRun,
        hasResult: stats.queried,
...
async function main() {
  ...
  console.log(
    `本次處理：${keywords.length} 個（第 ${startIndex + 1}～${startIndex + keywords.length} 個）`,
  );
  ...
  for (let i = 0; i < keywords.length; i++) {
    ...
    process.stdout.write(
      `[${globalIdx + 1}/${allKeywords.length}] 查詢「${kw}」... `,
    );
