...
if (!allDistrictsMode && (!cityArg || !districtArg)) {
  if (!listMode) {
    console.error("Usage:");
    console.error("  Single district: --city <city> --district <district> [--dry-run] [--delay <ms>]");
    console.error("  All districts:   --all-districts [--limit <n>] [--resume] [--only-city <city>] [--list] [--delay <ms>]");
    process.exit(1);
  }
}

// ── Data ─────────────────────────────────────────────────────────────────────
const DISTRICTS_FILE = path.resolve(__dir, "../../data/cvs/taiwan-city-districts.json");
...
function loadAllDistricts() {
  if (!fs.existsSync(DISTRICTS_FILE)) throw new Error(`行政區清單不存在：${DISTRICTS_FILE}`);
...
async function queryEmapDistrict(city, district) {
  const body = new URLSearchParams({ commandid: "SearchStore", city, town: district });
  ...
  if (dryRun) {
    ...
    try {
      ...
      stores.slice(0, 5).forEach((s, i) =>
        console.log(`  ${i + 1}. ${s.storeId} ${s.storeName}  ${s.storeAddress}`)
      );
  ...
  try {
    ...
    for (const store of stores) {
      try {
        ...
        if (action === "inserted") { inserted++; process.stdout.write("+"); }
...
async function runAllDistricts() {
  ...
  if (onlyCityArg) {
    ...
    console.log(`--only-city ${onlyCityArg}：篩選後 ${allDistricts.length} 個行政區`);
  }
  ...
  if (resumeMode && progress) {
    ...
    allDistricts = allDistricts.filter(d => !completedKeys.has(`${d.city}|${d.district}`));
    console.log(`--resume：已完成 ${completedKeys.size} 個，略過後剩 ${allDistricts.length} 個`);
  } else if (!resumeMode) {
    progress = null;
  }
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    process.stdout.write(`[${i + 1}/${toProcess.length}] ${city} ${district} ... `);
    ...
    let districtInserted = 0, districtUpdated = 0, districtFailed = 0, returned = 0;
    let dbTotalAfter = 0;
    ...
    try {
      ...
      console.log(`returned=${returned} +${districtInserted} ~${districtUpdated} !${districtFailed}  DB=${dbTotalAfter}`);
      ...
      globalStats.districtReports.push({ city, district, status: "failed", error: err.message });
    }
    ...
    // Save running report
    saveReport({ ...globalStats, completedDistricts: [...completedKeys], failedDistricts });
  ...
  if (failedDistricts.length > 0) {
    ...
    failedDistricts.forEach(f => console.log(`  - ${f.city} ${f.district}：${f.error}`));
  }
  console.log(`──────────────────────────────────────────────────\n`);

  if (!dryRun) {
    saveReport({ ...globalStats, completedDistricts: [...completedKeys], failedDistricts, dbTotalFinal: finalDb });
  }
}
