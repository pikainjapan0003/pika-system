...
const REPORT_PATH = reportArgRaw
  ? (path.isAbsolute(reportArgRaw) ? reportArgRaw : path.resolve(WORKSPACE_ROOT, reportArgRaw))
  : DEFAULT_REPORT_PATH;
...
if (!dryRun && !upsertMode && !listCities) {
  ...
  console.error("       node scripts/import-family-stores-from-twcoupon.mjs --upsert --city X --district Y");
  process.exit(1);
}

// Upsert mode: require --city and --district, OR explicit --limit-cities for full upsert
const upsertAllMode = upsertMode && limitCitiesRaw != null && !cityArg && !districtArg;
if (upsertMode && !upsertAllMode && (!cityArg || !districtArg)) {
  console.error("Error: --upsert requires --city and --district to limit scope.");
...
let pool = null;
if (upsertMode) {
  if (!process.env.DATABASE_URL) throw new Error("DATABASE_URL must be set for --upsert mode");
...
function buildDistrictReport(city, district, url, enriched, deduped, duplicates) {
  ...
  if (missingPostalCode > 0) anomalies.push({ type: "missing_postal_code", count: missingPostalCode });
...
async function runDryRun() {
  ...
  for (let ci = 0; ci < citiesToProcess.length; ci++) {
    ...
    const cityMissingPostalCode = cityStores.filter(s => !s.postalCode).length;
    const cityDupCount = districtReports.reduce((sum, dr) => sum + dr.duplicateCount, 0);
  ...
  const { deduped: globalDeduped, duplicates: globalDups } = deduplicateStores(allStores);
  if (globalDups.length > 0) {
    reportData.duplicates.push(...globalDups);
    reportData.anomalies.push({ type: "cross_district_duplicate", count: globalDups.length });
  }
  ...
  console.log(`missingPostalCode： ${reportData.missingStats.missingPostalCode}`);
...
async function upsertStore(store) {
  const result = await pool.query(
    ...
    [store.generatedStoreId, store.storeName, store.storeAddress,
     store.phone ?? null, store.city ?? null, store.district ?? null]
  );
...
async function runUpsert() {
  ...
  const { districtReports, error } = await collectFromCity(cityArg, districtArg);
  ...
  const report = {
    ...
    duplicates,
    anomalies: failedList.length > 0 ? [{ type: "upsert_failed", count: failedList.length, detail: failedList }] : [],
  ...
  console.log("═".repeat(60));
  console.log(`[UPSERT] seven 筆數不變：${sevenBefore === sevenAfter ? "✓" : "✗（異常！）"}`);
  console.log("═".repeat(60));
...
async function runUpsertAll() {
  ...
  const allCityNames = cityUrls
    .slice(0, limitCities < Infinity ? limitCities : cityUrls.length)
    .map(extractCityNameFromUrl);
  console.log(`發現 ${cityUrls.length} 個縣市，本次處理 ${allCityNames.length} 個\n`);
  ...
  const reportData = {
    ...
    updated:       0,
    failed:        0,
    dbCounts:      { familyBefore, familyAfter: 0, sevenBefore, sevenAfter: 0, sevenUnchanged: false },
    cities:        [],
    districtReports: [],
    duplicates:    [],
    anomalies:     [],
  ...
  for (let ci = 0; ci < allCityNames.length; ci++) {
    ...
    for (const dr of districtReports) {
      ...
      if (dr.duplicates  && dr.duplicates.length  > 0) reportData.duplicates.push(...dr.duplicates);
      if (dr.anomalies   && dr.anomalies.length   > 0) {
        for (const a of dr.anomalies) reportData.anomalies.push({ city: dr.city, district: dr.district, ...a });
      }
    }
    ...
    console.log(`  ${cityName} 合計：${cityDeduped} 筆（行政區 ${districtReports.length} 個）`);
    reportData.cities.push({
      city:             cityName,
      districtCount:    districtReports.length,
      ...
      duplicateCount:   districtReports.reduce((sum, dr) => sum + dr.duplicateCount, 0),
    });
  }
  ...
  for (let i = 0; i < globalDeduped.length; i++) {
    ...
    if ((i + 1) % 200 === 0) {
      console.log(`  進度：${i + 1}/${globalDeduped.length}（inserted ${totalInserted}，updated ${totalUpdated}，failed ${totalFailed}）`);
    }
  }
  ...
  reportData.dbCounts  = { familyBefore, familyAfter, sevenBefore, sevenAfter, sevenUnchanged: sevenBefore === sevenAfter };
  reportData.cityCount     = reportData.cities.length;
  reportData.districtCount = reportData.districtReports.length;
  reportData.samples   = globalDeduped.slice(0, 20);
  reportData.missingStats = {
  ...
  if (failedList.length > 0) {
    reportData.anomalies.push({ type: "upsert_failed", count: failedList.length, detail: failedList });
  }
  ...
  console.log(`seven 不變：        ${sevenBefore === sevenAfter ? "✓" : "✗（異常！）"}`);
  reportData.cities.forEach(c => {
    console.log(`  ${c.city}：${c.storeCount ?? 0} 筆（${c.districtCount ?? 0} 個行政區）`);
  });
  console.log("═".repeat(60));
