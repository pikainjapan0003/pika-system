...
test("override wins over dates", () => {
  ...
  assert.deepEqual(result, { status: "ready", workingDays: 10n, source: "override" });
});
...
test("invalid or reversed dates are pending", () => {
  assert.equal(
    resolveWorkingDays({ startDate: "2026-02-30", endDate: "2026-03-01" }).status,
    "pending_confirmation",
  );
  assert.equal(
    resolveWorkingDays({ startDate: "2026-08-03", endDate: "2026-08-01" }).status,
    "pending_confirmation",
  );
});
