...
function input(overrides = {}) {
  return {
    ...
    transport: { product, route, trip, area, areaCost },
    ...overrides,
  };
}
...
test("order-time 小樽 snapshot captures the approved exact chain at 12 decimal places", () => {
  ...
  // 800 - 591.03 - 16.561340663997933... = 192.408659336002066...;
  // full profit = 208.97.
  assert.deepEqual(calculateOrderProfitSnapshot(input()), {
    ...
    profitSnapshotTransportCostTwd: "16.561340663998",
    profitSnapshotUnitProfitTwd: "192.408659336002",
...
test("missing cost is captured as pending without silently writing zero", () => {
  ...
  assert.deepEqual(
    createInitialOrderProfitSnapshot(input({ costJpy: null }), capturedAt),
    {
...
test("transport-exempt snapshot stores zero transport with an explicit exempt status", () => {
  const snapshot = calculateOrderProfitSnapshot(
    input({
      unitPriceTwd: "1900",
      costJpy: "8000",
...
test("pending snapshot can be backfilled once and is then permanently rejected", () => {
  ...
  const first = backfillPendingOrderProfitSnapshot(
    "pending",
    input(),
    backfilledAt,
  );
  assert.equal(first.outcome, "backfilled");
  assert.equal(first.values.profitSnapshotUnitProfitTwd, "192.408659336002");
...
test("captured order stays frozen when the current store rate changes", () => {
  ...
  const currentEstimate = calculateOrderProfitSnapshot(
    input({
      storePurchaseExchangeRate: "0.21",
    }),
  );
  ...
  assert.equal(captured.profitSnapshotUnitProfitTwd, "192.408659336002");
  assert.equal(
    currentEstimate.profitSnapshotProductCostTwd,
    "623.700000000000",
  );
  assert.equal(currentEstimate.profitSnapshotUnitProfitTwd, "159.738659336002");
  assert.equal(captured.profitSnapshotUnitProfitTwd, "192.408659336002");
});

test("Q69 captures a repeating decimal at 12 places with half-up rounding", () => {
  // est_qty=3、手動 ETC=90、train=1；路線四項費基 = 91，手續費 = 1.365；
  // transport = (90+1+1.365) ÷ 3 = 92.365/3 = 30.78833...；
  // unit profit = 100 - 92.365/3 = 69.21166...，第 13 位使第 12 位進位。
  ...
  assert.equal(snapshot.profitSnapshotTransportCostTwd, "30.788333333333");
  assert.equal(snapshot.profitSnapshotUnitProfitTwd, "69.211666666667");
