...
test("order-time 小樽 snapshot captures the approved exact chain at 12 decimal places", () => {
  ...
  // 800 - 591.03 - 22.61095834375 = 186.35904165625; full profit = 208.97.
  assert.deepEqual(calculateOrderProfitSnapshot(input()), {
    ...
    profitSnapshotTransportCostTwd: "22.610958343750",
    profitSnapshotUnitProfitTwd: "186.359041656250",
...
test("pending snapshot can be backfilled once and is then permanently rejected", () => {
  ...
  assert.equal(first.values.profitSnapshotUnitProfitTwd, "186.359041656250");
...
test("captured order stays frozen when the current store rate changes", () => {
  ...
  assert.equal(captured.profitSnapshotUnitProfitTwd, "186.359041656250");
  assert.equal(
    currentEstimate.profitSnapshotProductCostTwd,
    "623.700000000000",
  );
  assert.equal(currentEstimate.profitSnapshotUnitProfitTwd, "153.689041656250");
  assert.equal(captured.profitSnapshotUnitProfitTwd, "186.359041656250");
});

test("Q69 captures a repeating decimal at 12 places with half-up rounding", () => {
  // est_qty=3、手動 ETC=90、train=1；六項費基 = 91，手續費 = 1.365；
