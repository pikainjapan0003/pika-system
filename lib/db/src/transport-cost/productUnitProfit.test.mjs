...
test("Fixture B: 小樽 transport is deducted from unit profit but not full profit", () => {
  ...
  // System-formula regression fixture: 22.6109583 TWD.
  // Unit profit: 800 - 591.03 - 22.6109583 = 186.3590417.
  // 來源 Sheet 為 26.06，2026-08-08 已降為參考；權威為 26.03。見 PROJECT_PLAN §20.1 A。
  ...
  assert.equal(result.unitTransportCostTwd.toDecimalPlaces(7), "22.6109583");
  assert.equal(result.unitProfitTwd.toDecimalPlaces(7), "186.3590417");
  assert.equal(result.fullUnitProfitTwd.toDecimalPlaces(2), "208.97");
  assert.equal(result.displayUnitProfitTwd, "186");
...
test("Fixture D: changing the store rate immediately recalculates profit without caching transport", () => {
  ...
  assert.equal(original.unitProfitTwd.toDecimalPlaces(7), "186.3590417");
  assert.equal(changed.productCostTwd.toDecimalPlaces(1), "623.7");
  assert.equal(changed.unitTransportCostTwd.toDecimalPlaces(7), "22.6109583");
  assert.equal(changed.unitProfitTwd.toDecimalPlaces(7), "153.6890417");
