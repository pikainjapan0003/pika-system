...
test("Fixture B: route and Hokkaido area transport are deducted from unit profit but not full profit", () => {
  // Purchase calculation: 2970 × 0.199 = 591.03.
  // System-formula regression fixture: route 13.14796109375 TWD plus
  // Hokkaido area 3.413379570247933... TWD = 16.561340663997933... TWD.
  // Unit profit: 800 - 591.03 - 16.561340663997933... = 192.408659336002066....
  // Route inputs remain a system-formula regression lock; the area inputs use
  // the authoritative 26.03 Hokkaido row. See PROJECT_PLAN §20.1 A/F.
  ...
  assert.equal(result.unitTransportCostTwd.toDecimalPlaces(7), "16.5613407");
  assert.equal(result.unitProfitTwd.toDecimalPlaces(7), "192.4086593");
  assert.equal(result.fullUnitProfitTwd.toDecimalPlaces(2), "208.97");
  assert.equal(result.displayUnitProfitTwd, "192");
...
test("Fixture D: changing the store rate immediately recalculates profit without caching transport", () => {
  ...
  assert.equal(original.unitProfitTwd.toDecimalPlaces(7), "192.4086593");
  assert.equal(changed.productCostTwd.toDecimalPlaces(1), "623.7");
  assert.equal(changed.unitTransportCostTwd.toDecimalPlaces(7), "16.5613407");
  assert.equal(changed.unitProfitTwd.toDecimalPlaces(7), "159.7386593");
