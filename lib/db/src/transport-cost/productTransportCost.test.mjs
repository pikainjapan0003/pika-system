...
test("the same attached product dynamically reflects changed route inputs", () => {
  ...
  assert.equal(original.finalCostPerItem.toDecimalPlaces(8), "22.61095834");
  assert.equal(changed.finalCostPerItem.toDecimalPlaces(8), "23.87336459");
  assert.equal(
...
test("Q61: product transport cost uses only trip.exchange_rate", () => {
  ...
  assert.equal(result.finalCostPerItem.toDecimalPlaces(8), "22.72458125");
});
