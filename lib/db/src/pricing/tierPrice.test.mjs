...
test("an explicitly configured zero tier price remains zero instead of falling back", () => {
  assert.equal(resolveTierPrice({ ...prices, vipPrice: "0", customerTier: "vip" }).priceTwd, "0");
});

test("approved VIP and partner prices are selected exactly", () => {
  assert.equal(resolveTierPrice({ ...prices, customerTier: "vip" }).priceTwd, "1800");
  assert.equal(resolveTierPrice({ ...prices, customerTier: "partner" }).priceTwd, "1500");
});
...
test("tier price profit delegates to the existing exact product profit chain", () => {
  ...
  const general = calculateTierProductUnitProfit({ ...base, customerTier: null });
...
test("negative tier prices are rejected before profit calculation", () => {
  assert.throws(() => resolveTierPrice({ ...prices, vipPrice: "-1", customerTier: "vip" }), /price cannot be negative/);
});
