...
export const profitSnapshotStatusEnum = [
  "captured",
  "pending",
  "exempt",
] as const;
...
export const ordersTable = pgTable(
  "orders",
  {
    ...
    profitSnapshotCostJpy: numeric("profit_snapshot_cost_jpy", {
      precision: 30,
      scale: 12,
    }),
    profitSnapshotExchangeRate: numeric("profit_snapshot_exchange_rate", {
      precision: 30,
      scale: 12,
    }),
    profitSnapshotProductCostTwd: numeric("profit_snapshot_product_cost_twd", {
      precision: 30,
      scale: 12,
    }),
    profitSnapshotTransportCostTwd: numeric(
      "profit_snapshot_transport_cost_twd",
      { precision: 30, scale: 12 },
    ),
    profitSnapshotUnitProfitTwd: numeric("profit_snapshot_unit_profit_twd", {
      precision: 30,
      scale: 12,
    }),
    profitSnapshotFullUnitProfitTwd: numeric(
      "profit_snapshot_full_unit_profit_twd",
      { precision: 30, scale: 12 },
    ),
    profitSnapshotStatus: text("profit_snapshot_status"),
    profitSnapshotCapturedAt: timestamp("profit_snapshot_captured_at", {
      withTimezone: true,
    }),
    profitSnapshotBackfilledAt: timestamp("profit_snapshot_backfilled_at", {
      withTimezone: true,
    }),
    ...
    cartProfitSnapshotTotalTwd: numeric("cart_profit_snapshot_total_twd", {
      precision: 30,
      scale: 12,
    }),
  ...
  (t) => [
    ...
    check(
      "orders_status_valid",
      sql`${t.status} IN ('pending', 'awaiting_payment', 'preparing', 'shipped', 'completed', 'cancelled')`,
    ),
    check(
