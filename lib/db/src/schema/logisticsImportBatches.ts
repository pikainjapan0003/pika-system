...
export type LogisticsImportProvider =
  (typeof logisticsImportProviderEnum)[number];

export const logisticsImportBatchStatusEnum = [
  "dry_run",
  "confirmed",
  "cancelled",
  "failed",
] as const;
export type LogisticsImportBatchStatus =
  (typeof logisticsImportBatchStatusEnum)[number];

// 每次老闆上傳物流 Excel 的 dry-run 批次。confirm（Step 7B-LOGISTICS-IMPORT-CONFIRM）
// 必須以已落地的 batch + rows 為準，不可重新解析檔案，確保確認內容與 dry-run 一致。
export const logisticsImportBatchesTable = pgTable(
  "logistics_import_batches",
  {
  ...
  (t) => [
    ...
    check(
      "logistics_import_batches_provider_valid",
      sql`${t.provider} IN ('711', 'familymart')`,
    ),
    check(
      "logistics_import_batches_status_valid",
      sql`${t.status} IN ('dry_run', 'confirmed', 'cancelled', 'failed')`,
    ),
  ],
);

export const insertLogisticsImportBatchSchema = createInsertSchema(
  logisticsImportBatchesTable,
).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertLogisticsImportBatch = z.infer<
  typeof insertLogisticsImportBatchSchema
>;
export type LogisticsImportBatch =
  typeof logisticsImportBatchesTable.$inferSelect;
