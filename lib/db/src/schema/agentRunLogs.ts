...
export const agentRunLogsTable = pgTable("agent_run_logs", {
  ...
  tokenId: integer("token_id").references(() => sellerAgentTokensTable.id, { onDelete: "set null" }),
  ...
  index("agent_run_logs_merchant_id_store_id_idx").on(t.merchantId, t.storeId),
  ...
  check("agent_run_logs_run_type_valid", sql`${t.runType} IN ('manual', 'scheduled', 'webhook', 'csv_after_import', 'test')`),
  check("agent_run_logs_status_valid", sql`${t.status} IN ('running', 'completed', 'failed', 'partial')`),
  check("agent_run_logs_counts_non_negative", sql`${t.checkedCount} >= 0 AND ${t.successCount} >= 0 AND ${t.failedCount} >= 0`),
]);

export const insertAgentRunLogSchema = createInsertSchema(agentRunLogsTable).omit({ id: true, createdAt: true });
