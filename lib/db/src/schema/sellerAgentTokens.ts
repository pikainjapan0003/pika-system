...
export type SellerAgentTokenStatus =
  (typeof sellerAgentTokenStatusEnum)[number];

export const sellerAgentTokensTable = pgTable(
  "seller_agent_tokens",
  {
    ...
    scopes: jsonb("scopes")
      .notNull()
      .default(["tracking:read", "tracking:write", "run_log:write"]),
  ...
  (t) => [
    ...
    index("seller_agent_tokens_merchant_id_store_id_idx").on(
      t.merchantId,
      t.storeId,
    ),
    ...
    check(
      "seller_agent_tokens_status_valid",
      sql`${t.status} IN ('active', 'revoked', 'expired', 'disabled')`,
    ),
  ],
);

export const insertSellerAgentTokenSchema = createInsertSchema(
  sellerAgentTokensTable,
).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertSellerAgentToken = z.infer<
  typeof insertSellerAgentTokenSchema
>;
