import { pgTable, text, serial, timestamp, integer, jsonb, index, check } from "drizzle-orm/pg-core";
...
export const logisticsImportRowsTable = pgTable("logistics_import_rows", {
  id: serial("id").primaryKey(),
  batchId: integer("batch_id").notNull().references(() => logisticsImportBatchesTable.id, { onDelete: "cascade" }),
  ...
  matchedOrderId: integer("matched_order_id").references(() => ordersTable.id, { onDelete: "set null" }),
  ...
  check("logistics_import_rows_match_status_valid", sql`${t.matchStatus} IN ('matched', 'needs_review', 'ambiguous', 'not_found', 'conflict', 'invalid', 'imported', 'skipped')`),
]);

export const insertLogisticsImportRowSchema = createInsertSchema(logisticsImportRowsTable).omit({ id: true, createdAt: true, updatedAt: true });
