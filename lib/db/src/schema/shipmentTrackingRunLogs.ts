...
export const trackingRunProviderEnum = [
  "711",
  "familymart",
  "tcat",
  "postoffice",
...
export const trackingRunStatusEnum = [
  "running",
  "success",
  "partial",
  "failed",
...
export const shipmentTrackingRunLogsTable = pgTable(
  "shipment_tracking_run_logs",
  {
  ...
  (t) => [
    ...
    check(
      "shipment_tracking_run_logs_run_type_valid",
      sql`${t.runType} IN ('scheduled_worker', 'manual_worker', 'adapter_health_check', 'import_confirm', 'exception_retry')`,
    ),
    check(
      "shipment_tracking_run_logs_provider_valid",
      sql`${t.provider} IS NULL OR ${t.provider} IN ('711', 'familymart', 'tcat', 'postoffice', 'all')`,
    ),
    check(
      "shipment_tracking_run_logs_status_valid",
      sql`${t.status} IN ('running', 'success', 'partial', 'failed', 'cancelled')`,
    ),
    check(
      "shipment_tracking_run_logs_counts_non_negative",
      sql`${t.totalJobs} >= 0 AND ${t.successCount} >= 0 AND ${t.failedCount} >= 0 AND ${t.skippedCount} >= 0`,
    ),
  ],
);

export const insertShipmentTrackingRunLogSchema = createInsertSchema(
  shipmentTrackingRunLogsTable,
).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertShipmentTrackingRunLog = z.infer<
  typeof insertShipmentTrackingRunLogSchema
>;
export type ShipmentTrackingRunLog =
  typeof shipmentTrackingRunLogsTable.$inferSelect;
