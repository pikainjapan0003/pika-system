...
test("customer input rejects a missing code or name and permits a missing phone", () => {
  ...
  assert.equal(validateCustomerInput({ code: "DEMO", name: "假名", phone: null }).phone, null);
  assert.equal(validateCustomerInput({ code: "DEMO", name: "假名" }).phone, null);
});
...
test("customer tier accepts the four approved values and rejects inventions", () => {
  for (const tier of ["general", "vip", "wholesale", "partner"]) {
    assert.equal(validateCustomerInput({ code: "DEMO", name: "假名", phone: "0912000000", tier }).tier, tier);
  }
  assert.throws(
    () => validateCustomerInput({ code: "DEMO", name: "假名", phone: "0912000000", tier: "gold" }),
    /tier must be/,
  );
});
