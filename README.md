...
- 客戶購物金發放／調整、訂單折抵與取消回沖；餘額以不可改寫的流水帳計算。
- 賣貨便訂單資格檢查、可匯出／不可匯出分區與明文二次確認後下載 CSV。
- 訂單揀貨／出貨、物流方式開關、付款末五碼人工對帳與物流異常工具。
- 老闆首頁重點、每月定格毛利報表，以及可預覽差異後套用的技能地圖。
- 台銀、土銀、合庫與一銀的日圓即期賣出參考匯率（只供手動套用，不會自動儲存；不可用時不回 0）。

金額欄遵守精確十進位運算；成本不足時顯示「待確認」，不以 0 代替。訂單快照建立或補拍後定格，之後修改商品成本或匯率不會改動舊單。

技能開關只調整後台日常入口的顯示；伺服器授權仍是獨立防線。零技能的新店會在首頁看到 4 題推薦問卷；問卷只做套餐推薦，仍須由店主確認套用，技能前置條件與高風險確認維持在技能地圖處理。

店主操作請看 [操作手冊](docs/ai-ops/23-operations-manual.md)。購物金、賣貨便 CSV／XLSM 下載與首登推薦問卷已可操作；XLSM 的 Excel 開檔與官方匯入仍須由老闆用假資料親自驗收。

## 開發測試

已驗證並納入主 CI 的瀏覽器測試放在 `e2e/suite/`；尚未驗證的候選測試只放在 `e2e/unverified/`，不會進入主 CI。GitHub Actions 的「Pending E2E」目前保留作人工重跑已驗證套件，失敗時會保留 7 天的 trace 與截圖。
...
示範資料腳本會建立假店鋪、假客戶、部分已開技能、成本商品、購物金流水，以及賣貨便可匯出／不可匯出的假訂單。唯一支援入口如下；它只接受命令列明確提供的拋棄式 PostgreSQL URL，並會拒絕含 `replit` 或 `prod` 字樣的目標：
